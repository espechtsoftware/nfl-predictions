...
**Backups (updated 2026-08-10).** `s-backup` runs `backup-tables` daily at 07:00 UTC: BigQuery snapshots of the irreplaceable tables (LineStar ownership backfill, standings imports, notes/watchlist, entered lineups, ID overrides, and licensed Fantasy Points imports) into the `nfl_backups` dataset, 30-day retention, delta-billed (~pennies). The current `fantasy_points_route_share` and `fantasy_points_advanced_prior` tables are explicit members; every future base table in `nfl_raw` named `fantasy_points_*` is also discovered automatically, while views and external tables are excluded. Everything else is re-ingestable from source, and BigQuery time travel covers the last 7 days on all tables regardless. Restore: `CREATE TABLE <dataset>.<table> CLONE nfl_backups.<table>_<YYYYMMDD>`. New irreplaceable non-Fantasy-Points tables must still be added to `backup.TABLES` (same discipline as `status.FEEDS`). Runs year-round; never pause it.

| When (2026) | Do |
|---|---|
| **Before Mon Aug 24 — paid-data weekly checklist (pending final evidence)** | After the corrected direct-role, Route Share union and any separately preregistered paid-data diagnostic mechanically determine which Fantasy Points inputs survive, update the in-app **Weekly guide** with only those adopted/prospective report names, exact export filters, weekday and pre-lock deadline. Wire their importer/refresh checks before Week 1 and verify every resulting `nfl_raw.fantasy_points_*` table appears in the daily backup manifest. Do not burden the operator with weekly downloads for rejected arms, and never use the predicted week's completed-game data. |
...
| **Every Mon/Tue after settlement** | Download DraftKings **Entry History CSV** and upload it on the Season page. Also download and upload the **full standings CSV for one target GPP per slate while DraftKings still offers it**; winner-only summaries cannot recover places 2--5, portfolio ranks, payout tiers, or realized ROI. Confirm the standings import retains ordered entries before deleting the source file. |
