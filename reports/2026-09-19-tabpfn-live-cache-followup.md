...
## Image source verified; target-week boundary identified

Content-verified image layers contain generator and feature bytes identical to the local production source. [Image-source receipt](reviews/evidence/2026-09-19-tabpfn-image-source.json). Upcoming-only support is present. The isolated refresh protocol is now [prepared](2026-09-19-tabpfn-scratch-refresh-protocol.md). Its preflight found 43 labeled current-week rows in the repaired training table, in addition to 863 prior-week rows. The generator’s all-nonnull-label upcoming context has no target-week filter; the rehearsal filters in SQL before reading labels. A live salary repair must also address that boundary. Only row/support counts were read, not current-week outcome values.
