...
v = verdict(p, m, t, f)
print(f"{v['status']} build inputs for {a.season} week {a.week}: projections {p.get('rows')} rows (skill {p.get('skill_rows')}, {p.get('age_minutes', float('nan')):.0f} min old); "
      f"market {m.get('line', '')[:160]}; tabpfn rows {rows_for_week} (weeks {t['weeks_present']}); files {'ok' if f['ok'] else f['reason']}")
...
if a.receipt:
    ...
    pathlib.Path(a.receipt).write_text(json.dumps({"schema": "build-inputs-gate/v1", "checked_utc": now.isoformat(), "season": a.season, "week": a.week,
        "verdict": v, "chosen_dose": dose, "chosen_dose_sha256": sha(a.chosen_dose), "contests_sha256": sha(a.contests), "tabpfn_table": a.tabpfn_table}, indent=2, default=str) + "\n")
