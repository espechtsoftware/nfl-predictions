#!/usr/bin/env python3
"""Mechanical validation for the frozen TabPFN SIS QB line cache pair."""
...
from nfl_dfs.research.tabpfn_sis_qb_line import SIS_QB_FEATURES, feature_contract


PREFIX = "TABPFN_SIS_QB_LINE_JSON="
...
def extract_report(path: Path) -> dict:
    ...
    if len(payloads) != 1:
        raise ValueError(
            f"expected one SIS QB line report in {path}; got {len(payloads)}")
    return payloads[0]
...
def validate_reports(
    ...
    checks = {
        ...
        "inherited_law_identity": (
            control.get("label_law") == "active_only"
            and treatment.get("label_law") == "active_only"
            and control.get("feature_law") == "base"
            and treatment.get("feature_law") == "base"
        ...
        "same_source_snapshots": (
            ...
            and control.get("sis_source", {}).get("source_run_ids")
            == ["sis-team-context-tranche-1-v1"]
        ),
        "exact_feature_contracts": (
            control.get("feature_columns") == control_features
            and treatment.get("feature_columns") == treatment_features
            and treatment_features == [*control_features, *SIS_QB_FEATURES]
        ),
        ...
        "same_coverage_audits": (
            control.get("active_qb_coverage")
            == treatment.get("active_qb_coverage")
        ),
        "active_qb_minimum_coverage": all(
            any(
                row.get("season") == season
                and row.get("support_rate", 0.0) >= 0.80
                for row in control.get("active_qb_coverage", [])
            )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--inherited-table", choices=("tabpfn_active_label_treatment_v2",),
        required=True)
...
def main() -> None:
    ...
    report_validation = validate_reports(
        reports["control"], reports["treatment"], args.code_sha, baseline)
    tables = {
        arm: query_df(
    ...
    output = {
        "disposition": (
            "tabpfn-sis-qb-line-caches-valid"
            if passes else "tabpfn-sis-qb-line-caches-invalid"),
        "passes": passes,
    ...
    if not passes:
        raise SystemExit("ABORT: SIS QB line cache validation failed")
    print(f"TABPFN_SIS_QB_LINE_VALIDATED {args.output}")
