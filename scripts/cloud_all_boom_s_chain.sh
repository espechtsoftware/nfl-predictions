...
OUT="$ROOT/reports/all-boom-selection-s-runs/$RUN_ID"
PREFIX=gs://nfl-predictions-503414-raw/research/all-boom-selection-s-runs/$RUN_ID
REUSED_JOB=atlas-minimal-c-s2023-w1-v1
EXECUTIONS="$OUT/executions.txt"
BUILD_ID=${1:?build id}
CODE_SHA=${2:?code sha}

[ -e "$EXECUTIONS" ] && { echo "ERROR: all-boom S ledger exists" >&2; exit 2; }

while :; do
  STATUS=$(gcloud builds describe "$BUILD_ID" --project "$PROJECT" \
    --format='value(status)' 2>/dev/null || echo "")
  printf '%s ALLBOOMS_BUILD status=%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${STATUS:-Unknown}"
  case "$STATUS" in
    SUCCESS) break ;;
    FAILURE|CANCELLED|TIMEOUT|EXPIRED|INTERNAL_ERROR)
      echo "ERROR: all-boom S build terminal $STATUS" >&2; exit 2 ;;
    *) sleep 120 ;;
  esac
done
...
for RELATIVE in Dockerfile cloudbuild.yaml \
  ...
  reports/2026-08-19-all-boom-selection-s-protocol.md; do
  CURRENT=$(sha256sum "$ROOT/$RELATIVE" | awk '{print $1}')
  BUILT=$(git -C "$ROOT" show "$CODE_SHA:$RELATIVE" | sha256sum | awk '{print $1}')
  [ "$CURRENT" = "$BUILT" ] || {
    echo "ERROR: all-boom S built source differs: $RELATIVE" >&2; exit 2; }
done
if gsutil -q stat "$PREFIX/slate-*.json" 2>/dev/null; then
  echo "ERROR: all-boom S prefix already holds objects" >&2; exit 2
fi
mkdir -p "$OUT"
...
gcloud run jobs deploy "$REUSED_JOB" --project "$PROJECT" --region "$REGION" \
  ...
  --command python \
  --args "scripts/run_all_boom_selection_s.py,--season,2023,--week,1,--output-uri,$PREFIX/slate-2023-1.json" \
  --quiet >/dev/null
echo "ALLBOOMS_JOB_UPDATED $IMAGE"
...
run_cell() {
  ...
  execution=$(gcloud run jobs execute "$REUSED_JOB" --project "$PROJECT" \
    ...
    --args "scripts/run_all_boom_selection_s.py,--season,$season,--week,$week,--output-uri,$uri")
  [[ "$execution" == "$REUSED_JOB-"* ]] || {
    echo "ERROR: all-boom S execution identity missing" >&2; exit 2; }
...
echo "ALLBOOMS_CANARY_LAUNCHED $CANARY"
DEADLINE=$(( $(date +%s) + 7200 ))
while :; do
  STATE=$(gcloud run jobs executions describe "$CANARY" \
    --project "$PROJECT" --region "$REGION" \
    --format='value(status.conditions[0].status)' 2>/dev/null || echo "")
  printf '%s ALLBOOMS_CANARY state=%s\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${STATE:-Unknown}"
  [ "$STATE" = "True" ] && break
  [ "$STATE" = "False" ] && {
    echo "ERROR: all-boom S canary failed; halt and disposition" >&2; exit 2; }
  [ "$(date +%s)" -ge "$DEADLINE" ] && {
    echo "ERROR: all-boom S canary exceeded two hours" >&2; exit 2; }
  sleep 60
done
gsutil -q stat "$PREFIX/slate-2023-1.json" || {
  echo "ERROR: all-boom S canary produced no output" >&2; exit 2; }
echo "ALLBOOMS_CANARY_PASSED"
...
[ "$(wc -l < "$EXECUTIONS")" = 54 ] || {
  echo "ERROR: all-boom S population is not 54" >&2; exit 2; }
printf '%s\n' \
  "run_id=$RUN_ID" "image=$IMAGE" "code_sha=$CODE_SHA" "build_id=$BUILD_ID" \
  "output_prefix=$PREFIX" \
  "protocol_sha256=$(sha256sum "$ROOT/reports/2026-08-19-all-boom-selection-s-protocol.md" | awk '{print $1}')" \
  "runner_sha256=$(sha256sum "$ROOT/scripts/run_all_boom_selection_s.py" | awk '{print $1}')" \
  ...
  'predeclared_prior=favorable-c-cleared-s-uncertain' 'cells=54' 'canary=2023-1' \
  > "$OUT/manifest.txt"
sha256sum "$OUT/manifest.txt" "$EXECUTIONS" > "$OUT/launch.sha256"
echo "ALLBOOMS_GRID_LAUNCHED"

while :; do
  OBJECTS=$(gsutil ls "$PREFIX/slate-*.json" 2>/dev/null | wc -l)
  printf '%s ALLBOOMS_GRID objects=%s/54\n' \
  ...
  while read -r SEASON WEEK JOB EXECUTION URI; do
    ...
    STATE=$(gcloud run jobs executions describe "$EXECUTION" \
      ...
      --format='value(status.conditions[0].status)' 2>/dev/null || echo "")
    if [ "$STATE" = "False" ]; then
      printf '%s ALLBOOMS_CELL_FAILED %s %s %s\n' \
...
if len(cells) != 54:
    raise SystemExit(f"ERROR: all-boom S downloaded {len(cells)} cells")
