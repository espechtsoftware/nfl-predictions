"""Fresh exposure-cap comparison; greedy-order prefixes and fixed slot-zero stresses."""
import argparse
import hashlib
...
BOOK_SHA = '53bba4df0a9a5394323ea8bbf39273b11503a77e50d1c854f1e0e7a9e3e042eb'
PAIRS = {
    'raw_cap48_minus_raw': ('raw_cap48', 'raw'),
    'raw_cap38_minus_raw': ('raw_cap38', 'raw'),
    'pmix_cap48_minus_pmix': ('pmix_cap48', 'pmix'),
    'pmix_cap38_minus_pmix': ('pmix_cap38', 'pmix'),
}
...
def main():
    ...
    bp = ROOT / 'exposure-cap-proposals/frozen-books.json'
    assert sha(bp) == BOOK_SHA
    frozen = json.loads(bp.read_text())
    books = frozen['books']
    assert len(books) == 7 and frozen['all_books_legal'] and not frozen['audits_read']
    audit_path = ROOT / 'exposure-cap-audit/receipt.json'
    audit = json.loads(audit_path.read_text())
    assert audit['both_selection_banks_exact'] and audit['frozen_books_sha256'] == BOOK_SHA
    assert audit['source_sha256'] == sha(HERE / '2026-09-19-d6400-exposure-cap-audit-banks.py')
    ...
    for name, b in books.items():
        ...
        if '_cap' in name:
            assert np.bincount(roster[b].ravel()).max() <= int(name.split('_cap')[1])
    banks = {}
    for name, r in audit['banks'].items():
        bank_path = ROOT / 'exposure-cap-audit' / (name + '_audit.npy')
        assert sha(bank_path) == r['sha256'] and r['seed'] == frozen['audit_seeds'][name]
        banks[name] = np.load(bank_path, allow_pickle=False)
    ...
    raw = (ROOT / 'milly_winners.json').read_bytes()
    ...
    result = dict(reader_sha256=sha(__file__), frozen_books_sha256=BOOK_SHA,
        ...
        all_seven_arms_reported=True, no_audit_selected_book=True, current_outcomes_read=False,
        scope='Fixed-law cap utility/slot-zero comparison; cap and raw/pmix prefix/block orders are before delivery vetting, not contest-allocation claims.',
        primary=frozen['primary'], order_stage=frozen['order_stage'],
        seconds=time.monotonic() - started)
    with output.open('x') as f:
        json.dump(result, f, indent=2, allow_nan=False)
        f.write('\n')
    print(json.dumps({b: {r: contrasts[b]['participation_mixture'][r] for r in ('prefix1', 'prefix30', 'prefix97')}
        for b in PAIRS}, indent=2))
