#!/usr/bin/env python
"""Audit and score the frozen SIS RB run-tail five-seed exact-80 pair."""
...
OUTPUT_CHUNK_PREFIX = "TABPFN_SIS_RB_RUNTAIL_EXACT80_V1_CHUNK="
OUTPUT_CHUNK_SIZE = 80_000
BOOTSTRAP_SEED = 8_142_028
...
def _schedules(value: str) -> dict[int, str]:
    payload = json.loads(value)
    schedules = {int(season): str(spec) for season, spec in payload.items()}
    experiment.validate_schedule_specs(schedules)
    return schedules


def _decoded_schedules(value: str) -> dict[int, str]:
    return _schedules(base64.b64decode(value, validate=True).decode("utf-8"))
...
def _score_report(frames: dict[tuple[str, int], object]) -> dict:
    ...
    for replicate in experiment.SEEDS:
        ...
        candidate_overlap = left.merge(
            right, on=["season", "week", "players"]
        ).groupby(["season", "week"]).size().reindex(delta.index, fill_value=0)
        paired.append({
            ...
            "mean_candidate_overlap": float(candidate_overlap.mean()),
        })
        large.extend({
    ...
    for season in experiment.SEASONS:
        ...
        for arm in ("control", "treatment"):
            values = [weekly[(arm, replicate)].loc[season]
                      for replicate in experiment.SEEDS]
            ...
            item[arm] = {
                "selected_tail": {
                    str(tail): int((combined >= tail).sum())
                    for tail in experiment.TAILS
                },
...
def main() -> int:
    ...
    control_schedules = _decoded_schedules(args.control_schedules_b64)
    treatment_schedules = _decoded_schedules(args.treatment_schedules_b64)
    ...
    report = {
        "protocol": "2026-08-14-sis-rb-runtail-five-seed-exact80",
