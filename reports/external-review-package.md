# External review package — NFL DFS system (2026-08-04)

## Instructions for the reviewing model

You are an independent reviewer of a DraftKings NFL DFS prediction and
lineup-construction system. The materials below are: (1) the project's
operating summary, (2) the last ~45 commits, (3) the FULL experiment
ledger (50 addenda) covering every A/B test, adoption, rejection, and
audit. Review for:

1. **Methodological soundness**: are the validation rules (six-season
   panels, co-run controls on every table rebuild, deterministic
   replays, walk-forward only) sufficient? Where could the verdicts
   still be fooled (multiple comparisons, order-luck selection,
   survivorship in the adoption sequence)?
2. **Statistical rigor**: adoption thresholds are small integer deltas
   (+2 to +6 tail weeks out of 107). Which adoptions look robust vs
   noise-level given the documented +/-5 order-luck band? Be specific.
3. **Hidden risks**: train/serve skew, leakage, overfitting-to-2025,
...
## 1. Project operating summary (CLAUDE.md)
...
```
a73d1e3 TabPFN generator versioned in-repo + upcoming-week live mode
cc64dc8 Runbook: tabpfn-gen cache regeneration (weekly + post-rebuild)
293641b ADOPT TabPFN marginals default-on + Addendum 50 (STPFN verdict)
cc6a978 MPG4 verdict: noise-band mixed, retires neutral
4e81199 =study
c7d083f ADOPT SCHED features + Addendum 49 (final panel verdicts)
a2a224f Per-contest fill (contest_id filter) + README upload-flow reference
4c8ed62 Addendum 48: four-reviewer audit record
80f4051 Audit fixes round 3: xfp train/serve skew, merge fanout guard, sim hygiene
f936052 Audit fixes round 2: locked-slot alignment, lock-aware assignment, Lev% scale
26cd477 Audit fixes round 1: thesis-batch crash, dead market-tails, CSV double build
23484c4 Sim-path parity audit: gate multiplier notes, restore kickoff, honest labels
4ccee94 'My notes' toggle: enable/disable converted watch-note tilts per build
bd6c68a TABPFN_MARGINALS lever: per-player TabPFN quantile marginals in the sim
b1c88f9 ADOPT QF: N_QB_VARIANTS=4 + OWN_MODEL=fade defaults (Addendum 47)
01ca441 LEM v1: factored model beats bigram (4.60 vs 8.12 NLL/event); road to sim recorded
4e0c485 LEM v0 verdict: composite vocab loses to bigram; factored v1 launched
7a7d901 Addendum 46: kNN comps null; env-forecast pipeline scaffold; CV gated
1831a6c Round-10 verdicts: cold-start pooling null; weather/synthetic staged
edc45e3 Market-implied prop distributions: module + /api/market-tails + Addendum 45
7e6b5ca LEM corpus builder + round-9 scoping (GPU-gated, eval bar pre-registered)
fb3b355 Causal vacated-capture candidates + Addendum 44 (event study)
d70dcf1 Addendum 43: TabPFN/conformal/persona offline verdicts
ba0aaf6 Preserve rounds-7/8 experiment scripts (TabPFN, conformal, persona)
4339d59 QD_CELLS: MAP-Elites archive candidate batch (research round 8)
8a959a4 Addendum 42: punt-shape panel nulls; DIRK8 severe regression
c44447b Addendum 42: showdown bring-back A/B verdict (wash, stays off)
df68852 Addendum 42: FINAL EXAM verdicts (QBVAR4 +2 leads; combos queued)
ee3c2b8 Showdown captain board: CPT/FLEX-optimal rates + P(top scorer) in UI
aa2b0c4 XFP + schedule-context candidate features (rebuild-gated)
6852c36 Thesis constraints: per-combo portfolio floors through sim path + API
db85601 Export selection order (entry_ix): one 150-entry run = full best-of-N curve
899ca40 Replay print: 187 qualifier line on the playable-weeks cut
5e0faab DIRICHLET_K env: the retune the K=20 null verdict itself requested
c460821 Salary-family redesigns: N_LOWSAL coverage batch; ALT_CEIL reclassified untested
921c395 Punt-threshold reimplementations: PUNT_SLOPE (shaped) + PUNT_STRICT (conditional)
ccf2cfb UI: set sim-mode build-time expectation (~1-4 min)
71b213e Adopt LIVE_SIMS=30k for live paths (sim-mode + weekly projections)
da3c123 OWN_MODEL=fade: unbundle decision input from measurement instrument
77c1b0f Chat UI: typing indicator + fixed-height scrolling layout
0ad1dcb Research-triage levers: showdown bring-back (88% of winners; rule was absent) + salary-barbell VALUE2
43e4349 Data audit fixes: team-code normalization (recovers ~1,500 training rows), RATE_DENOM_WEIGHTS lever, deficiency log
fc1f918 MAX_PER_GAME lever: winners spread across 5.3 games, max ~3 from one
d20b3d3 PUNT_MAX env lever: dose-test the inherited $4k punt threshold
437ff0b CFB export-links helper: one-click standings URLs from the fills poll

```

---

## 3. Full experiment ledger
