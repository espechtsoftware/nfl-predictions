#!/bin/bash
# Run and harvest the one frozen Route Share K=1 component-model diagnostic.
# Usage: bash scripts/cloud_fantasy_points_route_components.sh <IMAGE@sha256:...>
set -euo pipefail

IMG=${1:-}
PROJECT=nfl-predictions-503414
REGION=us-central1
RUN_ID=20260811-fp-route-components-v1
PANEL=20260810-lockfix-e80-k1-8677d21
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUT="$ROOT/reports/fantasy-points-route-component-runs/$RUN_ID"
...
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable Route component execution already recorded"; exit 2; }
...
JOB=fantasy-points-route-components
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  --image "$IMG" --command nfl-dfs \
  --args "fantasy-points-route-component-diagnostic,--panel,$PANEL" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: Route component job deployed $DEPLOYED, expected $IMG"; exit 1; }

EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: Route component execution id missing"; exit 1; }
...
LOG_FILTER+='textPayload:"FP_ROUTE_COMPONENT_JSON="'
...
prefix = "FP_ROUTE_COMPONENT_JSON="
payloads = []
...
if len(payloads) != 1:
    raise SystemExit(
        f"ABORT: expected one Route component report, got {len(payloads)}")
if not payloads[0].get("disposition") or not payloads[0].get("gate"):
    raise SystemExit("ABORT: Route component report is incomplete")
...
echo "Route component diagnostic complete: $EXEC ($OUT/report.json)"
