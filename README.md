...
| 2026-09-21 | `nfl_raw.weather` has carried the forecast `precip_prob` (percent) since the weather job started, but `020_game_weather.sql` selected only temperature, wind and dome, so no precipitation value reached the training/inference tables, the frame or the selector; the Week-2 MIN-CHI game carried 51% (snapshots 72% / 40% / 51% on 09-18/19/20) and produced the fewest skill points on the slate. Found by the laptop's end-to-end audit (ING-002). | Rain is invisible to projections and to the exposure sheet. | Plumbed through as pass-through columns (`precip_prob`, `weather_pulled_at`) on `production/weather-precip-plumbing-20260921`; not a model feature until a point-in-time weather shadow supports it. |
