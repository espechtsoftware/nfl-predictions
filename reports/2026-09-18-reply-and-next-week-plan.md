...
**Production follow-up:** a usable *pilot candidate* exists by filename under `gs://nfl-predictions-503414-raw/week1/prelock/2026-w01/a5-books/20260910t2315z-fa5d035/sources/D800_DEMAX/`: `candidates.parquet`, `frame.parquet`, `incumbent_player_scores.npy`, and `corrected_hsim_player_scores.npy`. D400 candidate/frame files also exist beside it. The whole Week-1 prefix has 35 objects (28 JSON, 4 parquet, 1 CSV, 2 NPY). No payloads opened. This supports an outcome-disabled full-pool mechanics/precision pilot **after identity/schema verification**, not an assumption that the stored banks are independent audit banks. It is one already-observed 2026 Week-1 slate and an older live code revision; it cannot establish historical D3200 effects or prospective confirmation. Newly independent audit worlds still require simulation.

Production's `cand_scores/` has 103 immediate prefixes; the only September-tagged prefix found is `prospective-cbwu-oi-2026w01-20260906T162624Z/`. Other production research/corpus roots were enumerated only at the directory level, not exhaustively scanned. No global absence claim is made about those archives or an unlisted workstation location. A missing file finding is not permission to regenerate a full panel.
...
## Follow-up source check: findings 3–5 are not fully closed at `769fc63b`

While waiting for the planning reply I inspected the actual patches (`fdc6345b`, `0002535f`). Several improvements are present, but three residual defects contradict a complete closure:

* **Bundle publication is still non-atomic.** `sunday_after_build.sh` stages and verifies, then executes `rm -f "$E"/*.csv "$E"/*.md; mv "$STAGE"/* "$E"/`. Each file is still moved separately into the directory being read. A filler can read files from different versions, or encounter missing files; a rename of a single resolved immutable bundle pointer or a shared producer/consumer lock is still needed. Staging protects against pre-publication generation failure, not concurrent publication races.
* **The entry watcher still advances `last="$sig"` unconditionally**, after either fill failure or Windows copy failure. A failed operation with unchanged inputs remains suppressed. Move success bookkeeping to successful full publication, with an idempotent retry policy. The after-build seen timing was improved; that does not fix the sibling watcher.
* **K90 verification is still weaker than the existing governed verifier.** The new SHA check explicitly accepts missing identity (`[:7] not in ("", sha[:7])`), does not reject dirty receipts, and omits lock, exact book-row/roster counts and required sidecar checks. The directory finder still accepts any matching concurrent receipt after start-minus-60; despite the comment, no upper timestamp check or unique invocation binding is implemented. Child nonzero exit is now correctly rejected. Please reuse the real receipt contract, reject missing identity, and capture/bind the exact invocation's output rather than claiming time-window matching identifies it.

These are static counterexamples in the pushed source, not claims about current live entry contents. Please reopen the affected portions of defects32–34 and include failure/concurrency tests. I have made no fixes or deployments; this follow-up is sent promptly because Sunday reliability is a prerequisite for the research plan.
