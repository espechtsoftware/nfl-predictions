...
def main():
    ...
    with data.outcome_firewall(2026):
        ...
        try:
            new_h=simulate_hsim(changed,2026,2,10000,seed=2326).astype(np.float32)
            new_a=simulate_hsim(changed,2026,2,10000,seed=2426).astype(np.float32)
        finally:
            world._games=original_games
        old_a=simulate_hsim(fr,2026,2,10000,seed=2426).astype(np.float32)
    outputs={}
    ...
    result=dict(input_receipt_sha256=args.input_sha256,input_table=receipt['table'],input_deltas=input_deltas,
        ...
        metrics=measures,verified_archive=verified,output_arrays=outputs,source_hashes=source_hashes,
