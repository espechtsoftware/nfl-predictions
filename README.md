...
**CFB DFS collection-only scaffold (2026-07-31, issue #13 item 7, owner request).** DraftKings now runs college football DFS slates (8 roster slots: QB/2RB/3WR/FLEX/Superflex). `ingest/cfb_job.py` (`nfl-dfs ingest-cfb`, gated by `INGEST_CFB_ENABLED`) polls the same undocumented endpoints `dk_job.py`/`contest_job.py` already use — `dk_client.cfb_draft_groups`/`cfb_contests`, sport code `CFB` — and lands salaries in `nfl_raw.cfb_dk_salaries` and contest fill snapshots in `nfl_raw.cfb_dk_contest_fills` (`sql/raw/006_cfb_dk.sql`), twin tables to `dk_salaries`/`dk_contest_fills` rather than shared ones so the validated NFL ingest path and its schema are untouched. Verified live 2026-07-31: DK's lobby recognizes `sport=CFB` as a real sport code (`getcontests?sport=CFB` returns `"SelectedSport":"CFB"`, vs. `null` for made-up codes) but no CFB draft groups exist yet in the draftgroups payload — college football preseason, none posted. So `cfb_draft_groups()` is implemented and unit-tested against a synthetic payload but **unexercised against a live CFB slate**; the discovery step the roadmap asked for (confirm the sport identifier survives contact with a real draft group once one posts) is still open. Collection only, as scoped: no season/week resolution, no features, no optimizer support — the point is a backtestable 2026-season dataset for a 2027 go/no-go on CFB DFS. Not wired into `deploy_jobs.sh` (same as the overlay scaffold below). Before adopting: once DK posts a real CFB slate (expected ~August), run `nfl-dfs ingest-cfb` by hand and confirm `cfb_dk_salaries` gets non-empty rows with sane salaries/positions for the 8-slot format, and that `dk_client._group_sport` is still finding `CFB` groups (schema could differ from the NFL shape in ways this scaffold hasn't seen).

**Overlay detection — polling scaffold, unvalidated (2026-07-31, issue #13 item 4).** `ingest/contest_job.py` (`nfl-dfs ingest-contests`, gated by `INGEST_CONTESTS_ENABLED`) polls DK's lobby contest list (`dk_client.nfl_contests`/`contests_frame`) and lands fill-rate/overlay snapshots in `nfl_raw.dk_contest_fills`: entries vs. `max_entries`, and for guaranteed contests, `overlay_dollars = max(prize_pool - entries*entry_fee, 0)` — free EV when the field hasn't filled a GTD pool DK backs regardless. Contests are filtered to the draft group IDs already known from `nfl_draft_groups()`, because DK's lobby tags off-season Madden-simulation and Best Ball contests with the same `sport=NFL` value as real slates and doesn't otherwise distinguish them (verified live 2026-07-31 against `https://www.draftkings.com/lobby/getcontests?sport=NFL`). This is infrastructure only: a single poll can't tell overlay from a contest that simply fills late the way real GPPs do, so it isn't wired into `deploy_jobs.sh`/the Cloud Scheduler cadence yet. Before adopting: run `nfl-dfs ingest-contests` by hand a few times an hour through a live slate's final approach to lock (a real DK contest confirmed to still show entries well under `max_entries` close to kickoff is the sanity check) and see whether `overlay_dollars` tracks anything an experienced player would recognize as a real overlay, not just early-lobby noise.
...
| 2026-07-31 | `dk_client.nfl_draft_groups()` filters `g.get("sport") == "NFL"`, but the live `DK_GROUPS` payload (verified 2026-07-31) nests the sport code under `contestType.sport` — no draft group in a live 180-group pull carried a top-level `sport` key at all | If this filter is actually matching zero groups against the live API today, `ingest-dk`'s hourly slate pull could be silently landing nothing; discovered incidentally while building the CFB scaffold (`cfb_draft_groups()` was written to check `contestType.sport` first, with a top-level fallback, specifically because of this). Not fixed here — item 7 says don't touch the validated NFL ingest path, and this repo's tests mock `nfl_draft_groups` rather than exercise it against a real payload, so it's unconfirmed whether this is a recent drift or the function has never matched live data | Open — next NFL ingest session should confirm against a live pull (e.g. an ad hoc `dk_client.nfl_draft_groups()` call in-season) before touching `dk_job.py`'s filter |
...
| `ingest_cfb` | Not scheduled — opt-in via `INGEST_CFB_ENABLED` (see *Known gaps*: CFB collection scaffold) | Collection only; would need a CFB slate calendar to schedule sensibly, unlike the fixed NFL week |
