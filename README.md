...
> **Current 80-entry production policy (2026-08-14):** K=1 role12 + boom40
...
> `classic-k1-role12-boom40-poscal-cbwu-v4`. Its licensed `CBWU` portfolio
> holds the total candidate budget fixed while sourcing it by deterministic
> quota/fill from five registered searches, then selects against five equal
> 10,000-world blocks. Every block is required; a partial build fails closed.
> The prior CE12/boom28 identity-scale
...
**Adopted classic money-lineup policy (2026-08-14).** The app and projection
job consume `inference/production_policy.py` directly: policy
`classic-k1-role12-boom40-poscal-cbwu-v4`, source experiment
`20260813-multiseed-candidate-world-v1`, baseline K=1 `tail_k1`
...
generation adds pre-selection candidates only. The `CBWU` transport runs the
five registered projection/role seed pairs, allocates exactly the R0 candidate
count across their score-blind deduplicated pools, cross-scores those rosters
in five equal 10,000-world blocks, and applies the unchanged greedy selector.
It retains the 80-entry candidate-generation basis when a smaller reserved
entry count is requested, and never imports the separately tested finite-K
pass-tail cache or schedules. The mapping overrides
...
**Off-season pause / season-start runbook (2026 edition).** Twenty-four schedulers are PAUSED for the off-season because they only re-process a finished season or require a live Sunday slate: `s-nflverse`, `s-features`, `s-features-route`, `s-train`, `s-train-k1`, `s-train-k1-role`, `s-train-k1-route`, `s-train-k1-route-role`, `s-project-tu`, `s-project-su`, `s-shadow-k1-early`, `s-shadow-k1-late`, `s-shadow-k1-nofloor-early`, `s-shadow-k1-nofloor-late`, `s-shadow-k3-early`, `s-shadow-k3-late`, `s-shadow-k1-roleunion-early`, `s-shadow-k1-roleunion-late`, `s-shadow-k1-route-roleunion-early`, `s-shadow-k1-route-roleunion-late`, `s-shadow-archetype-paired-early`, `s-shadow-archetype-paired-late`, `s-freeze-tail-early`, `s-freeze-tail-late`. Everything else (odds, DK poll, CFB scaffold, weather, freshness check) stays live year-round. The season-start sequence:

**Backups (updated 2026-08-13).** `s-backup` runs `backup-tables` daily at 07:00 UTC: BigQuery snapshots of the irreplaceable tables (LineStar ownership backfill, standings imports, notes/watchlist, entered lineups, ID overrides, and licensed Fantasy Points/SIS imports) into the `nfl_backups` dataset, 30-day retention, delta-billed (~pennies). The current `fantasy_points_route_share`, `fantasy_points_advanced_prior`, `fantasy_points_receiver_coverage_l4`, `fantasy_points_defense_coverage_l4`, `fantasy_points_advanced_passing_l4`, `fantasy_points_route_shape_l4`, and `fantasy_points_qb_shell_l4` tables are explicit members; every future base table in `nfl_raw` named `fantasy_points_*` or `sis_*` is also discovered automatically, while views and external tables are excluded. Everything else is re-ingestable from source, and BigQuery time travel covers the last 7 days on all tables regardless. Restore: `CREATE TABLE <dataset>.<table> CLONE nfl_backups.<table>_<YYYYMMDD>`. New irreplaceable tables outside those licensed namespaces must still be added to `backup.TABLES` (same discipline as `status.FEEDS`). Runs year-round; never pause it.

| When (2026) | Do |
|---|---|
| **Every Wed 10:00am CT — paid data + Odds API** | Run `source .venv/bin/activate && nfl-weekly-data run --week W`. The command verifies Fantasy Points, deliberately forces a fresh SIS logout/login, verifies the replacement session, then can be left unattended. It triggers the secret-backed Cloud Run `ingest-odds` job and captures the three prospective matchup reports before first kickoff. From Week 2 it also downloads only Fantasy Points source Week W-1 Route Share and performs its guarded hash-addressed archive/append; Week 1 automatically skips Route Share. If Week W-1 has not posted, retry Wednesday evening; finish before Thursday's `s-features-route` at 6:30am CT. The cloud Odds API schedules remain independent: game lines at 9:00am/3:00pm CT Wed-Sun and props Thursday 11:00am CT. SIS is preflighted but spends no queries unless a future evidence-approved recurring plan is passed with `--sis-plan`; never reuse closed historical tranches. Never use Week W results to predict Week W. Full behavior is in `automation/fantasy_points/README.md`. |
...
| **Before Mon Aug 24 — remove the forensic review corpus** | After the independent review is finished, delete the four manifest-bound tables from the production-inaccessible `nfl_forensic_review` dataset with `python scripts/cleanup_final_forensic_warehouse.py --manifest reports/final-forensic-runs/20260814-final-preseason-forensic-v1/freeze_manifest.json --confirm-manifest-sha <full manifest SHA> --receipt reports/final-forensic-runs/20260814-final-preseason-forensic-v1/warehouse_cleanup_receipt.json --delete`; then run the same command with `--verify-only` instead of `--delete`, commit/push the receipt, and only then resume the production schedulers. The deletion command requires all four exact schemas, manifest labels and expirations before it deletes anything; the verifier fails if a table reappears or the receipt differs. The tables have a 90-day automatic expiry only as a backstop—retaining them into the regular-season production window is forbidden. Production code reads only `nfl_raw`, `nfl_features` and `nfl_predictions`; it never reads this isolated dataset. |
| **Mon Aug 24** | Resume only through the fail-closed gate: `python scripts/resume_2026_production_schedulers.py --manifest reports/final-forensic-runs/20260814-final-preseason-forensic-v1/freeze_manifest.json --confirm-manifest-sha <full manifest SHA> --receipt reports/final-forensic-runs/20260814-final-preseason-forensic-v1/warehouse_cleanup_receipt.json` first performs a no-mutation preflight; repeat with `--resume` only after it passes. The command re-verifies all four forensic tables are absent, requires the exact cleanup receipt in pushed `origin/main`, and describes every scheduler before resuming any. Do not use the old raw scheduler loop because it bypasses this cleanup gate. Incumbent training first runs Tue Aug 25 and the isolated Route treatment first runs Thu Aug 27. Baseline/no-floor/K3 pools freeze Sunday-main at 10:30 and 11:20 CT; the role-union control and Route Share treatment freeze at 10:20 and 11:10 CT, then the delayed jobs freeze the predeclared incumbent selector memberships. |
