"""Fresh audit for the designation decomposition, with fixed replay-proved fitted laws."""
import hashlib
import json
import os
import runpy
...
LOCAL = ROOT / 'participation-designation-audit'
OUT = R / '2026-09-19-participation-designation-audit.json'
...
def main():
    ...
    with data.outcome_firewall(2026):
        ...
        fr = state['fr']
        saved = pd.read_parquet(state['out']/'frame.parquet')
        pd.testing.assert_frame_equal(saved,pd.read_parquet(baseout/'frame.parquet'))
        # The original CLI deliberately omits its normalized-name join helper from disk.
        assert set(fr.columns)-set(saved.columns) == {'nkey'}
        assert fr.nkey.equals(fr.name.map(state['norm_name']))
        pd.testing.assert_frame_equal(fr.drop(columns=['nkey']),saved)
        ...
        for component,draw,seeds in [('I',lambda seed:state['finish_bank'](state['model_draws'](seed)),
            ...
            for name,seed in seeds:
                ...
                print('DESIGNATION_AUDIT_BANK_SAVED',component,name,seed,flush=True)
    ...
    print('DESIGNATION_AUDIT_CONSTRUCTION_PASS',result['elapsed_seconds'],flush=True)
