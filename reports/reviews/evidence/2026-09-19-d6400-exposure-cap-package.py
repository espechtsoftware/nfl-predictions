...
OUT = BASE / 'd6400-exposure-cap-portable-v1'
...
def main():
    ...
    paths = ['source/frame.parquet', 'selection-parity/candidate_orders.json',
        'exposure-cap-proposals/frozen-books.json', 'exposure-cap-audit/receipt.json',
        'exposure-cap-audit/I_audit.npy', 'exposure-cap-audit/H_audit.npy']
    files = {}
    for rel in paths:
    ...
    for rel, src in {'expected-result.json': ROOT / 'exposure-cap-result.json',
                     'milly_winners.json': BASE / 'd6400-frontier-portable-v1/milly_winners.json'}.items():
        dst = OUT / rel
        shutil.copyfile(src, dst)
        files[rel] = dict(sha256=sha(dst), bytes=dst.stat().st_size)
    manifest = dict(schema='d6400-exposure-cap-numerical-replay/v1', files=files,
        original_result_sha256=sha(ROOT / 'exposure-cap-result.json'),
        replay_reader_sha256=sha(HERE / '2026-09-19-d6400-exposure-cap-replay-v2.py'),
        original_reader_sha256=sha(HERE / '2026-09-19-d6400-exposure-cap-read.py'),
    ...
    archive = BASE / 'd6400-exposure-cap-portable-v1.tar.gz'
