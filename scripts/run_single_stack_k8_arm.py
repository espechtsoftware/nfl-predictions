#!/usr/bin/env python3
"""Offline accounting scaffold for the contingent single-stack k=8 arm.

This script reads one canonical local JSON contract, calls the pure decision
module, and optionally writes one create-only local report.  It has no
BigQuery, GCS, Cloud Run, lease, generation, selection, or scoring body.
"""
...
from nfl_dfs.research.single_stack_k8_arm import (  # noqa: E402
    canonical_json,
    evaluate_payload,
)
...
def load_canonical(path: Path) -> object:
    ...
    try:
        ...
        raise ValueError("single-stack input is invalid JSON") from exc
    if canonical_json(value) != raw:
        raise ValueError("single-stack input is not canonical JSON")
    return value
...
def main() -> None:
    ...
    parser.add_argument("--input", required=True, help="canonical local JSON")
