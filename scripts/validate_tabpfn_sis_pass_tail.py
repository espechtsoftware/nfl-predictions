#!/usr/bin/env python3
"""Mechanical validation for the frozen TabPFN SIS pass-tail cache pair."""
...
PREFIX = "TABPFN_SIS_PASS_TAIL_JSON="
...
def extract_report(path: Path) -> dict:
    ...
    if len(payloads) != 1:
        raise ValueError(
            f"expected one SIS pass-tail report in {path}; got {len(payloads)}")
    return payloads[0]
...
def validate_reports(
    ...
    coverage = control.get("active_pass_tail_coverage", [])
    checks = {
        "arm_identity": (
        ...
        "exact_feature_contracts": (
            ...
            and treatment_features
            == [*control_features, *SIS_PASS_TAIL_FEATURES]
        ),
        ...
        "same_coverage_audits": (
            coverage == treatment.get("active_pass_tail_coverage")
        ),
        "active_pass_tail_minimum_coverage": all(
            any(
                row.get("season") == season
                and row.get("support_rate", 0.0) >= 0.80
                and set(row.get("by_position", {})) == {"QB", "WR", "TE"}
                for row in coverage
            )
...
def main() -> None:
    ...
    output = {
        "disposition": (
            "tabpfn-sis-pass-tail-caches-valid"
            if passes else "tabpfn-sis-pass-tail-caches-invalid"),
        "passes": passes,
    ...
    if not passes:
        raise SystemExit("ABORT: SIS pass-tail cache validation failed")
    print(f"TABPFN_SIS_PASS_TAIL_VALIDATED {args.output}")
