...
|---|---|---|---|
| 2026-08-08 | `raw.player_ids` is not unique on `gsis_id`; the replay's display-name join duplicated 31 otherwise unique 2014--2015 training player-weeks. Separately, sub-machine-precision component differences across Cloud Run CPU instances could cross discrete simulator RNG thresholds and break exact same-image reproduction | Duplicate rows slightly reweighted old training observations, while numerically equivalent model fits produced different simulated worlds, candidate rosters, and selected entries in 2019/2024 despite identical feature snapshots and actual scores | **Fixed; exact revalidation required** — replay selects one deterministic display name per GSIS id and orders DST rows; the shared live/replay simulator canonicalizes component means at 10 decimal places, fingerprints the raw and effective boundary, and requires a same-image exact-world proof before any arm verdict |
