"""Pure guards for the PIT-clean active-label exact-80 comparison."""
...
CONTROL_PANEL = "20260812-pitclean-e80-selected-tabpfn-current-v2"
...
def mechanism_failures(
    ...
    *,
    ...
    """Require cache plus its fitted schedule to be the only arm difference."""
    failures: list[str] = []
    ...
    if not control.code_sha.astype(str).eq(expected_code_sha).all() or not \
        ...
        failures.append("books do not share the frozen generation code SHA")
    ...
    if set(control_schedules) != expected_seasons or \
        ...
        failures.append("repaired active-label schedules are incomplete")
    ...
    for season in sorted(expected_seasons):
        ...
        for name, levers in (("control", left), ("treatment", right)):
            ...
            if not role_selected:
                ...
                if unexpected:
                    failures.append(
                        f"{name} {season} has unexpected role levers")
        ...
        left_common = {k: v for k, v in left.items() if k not in remove}
        right_common = {k: v for k, v in right.items() if k not in remove}
