...
GRAPH_SCHEMA_VERSION: Final = "corpus-graph-vnext/v2"
LOAD_MANIFEST_SCHEMA: Final = "foundry-graph-load-manifest/v2"
...
NODE_KINDS: Final = frozenset({
    ...
    "Question", "Claim", "Read", "Hold", "ExperimentDefinition",
    "Preregistration", "RunObservation", "ProposalAttemptAggregate",
})
...
ALLOWED_NAMESPACES: Final = frozenset({
    "identity", "membership", "trait", "metric", "lineage", "epistemic",
    "realized",
})
...
NODE_PROPERTY_SCHEMA: Final[dict[str, dict[str, PropertyRule]]] = {
    ...
    # Outcome-bearing kinds stay registered vocabulary but are closed in v2.
    ...
    "Question": {
        "source_id": _string(200), "text": _string(512),
        "status": _string(32), "decision_channel": _string(64),
        "note": _string(512),
    },
    "Claim": {
        "source_id": _string(200), "text": _string(512),
    ...
    "Read": {
        "source_id": _string(200),
        "preregistration_reference": _string(128),
        "verdict": _string(256), "endpoint_era": _string(64),
        "evidence_class": _string(64),
    },
    "Hold": {
        "source_id": _string(200), "reason": _string(512),
        "releaser": _string(256),
    },
    "ExperimentDefinition": {
        "source_id": _string(200), "name": _string(128),
    },
    "Preregistration": {
        "source_id": _string(200), "file_name": _string(256),
        "amendment_count": _integer(),
    },
    "ProposalAttemptAggregate": {
        "source_id": _string(200), "family": _string(64),
        "reported_attempt_count": _integer(),
        "reported_new_count": _integer(),
        "reported_duplicate_count": _integer(),
        "reported_reconciles": _boolean(),
        "reconciliation_verified": _boolean(),
    },
    "RunObservation": {
        "source_id": _string(200), "run_id": _string(200),
        "experiment_name": _string(128), "code_commit": _string(40),
        "image_digest": _string(128), "shard_count": _integer(),
        "slate_count": _integer(),
        "reported_reconciled_ledger_count": _integer(),
        "reported_ledger_violation_count": _integer(),
        "source_prefix_uri": _string(512),
        "verification_status": _string(
            32, allowed=frozenset({"reported_unverified"})
        ),
    },
}

NODE_PROPERTY_SCHEMA["Slate"].update({"source_id": _string(200)})
...
NODE_NAMESPACE_SCHEMA.update({
    ...
    "Question": frozenset({"epistemic"}),
    "Claim": frozenset({"epistemic"}),
    "Read": frozenset({"epistemic"}),
    "Hold": frozenset({"epistemic"}),
    "ExperimentDefinition": frozenset({"epistemic"}),
    "Preregistration": frozenset({"epistemic"}),
    "RunObservation": frozenset({"lineage"}),
    "ProposalAttemptAggregate": frozenset({"lineage"}),
})
...
RELATIONSHIP_NAMESPACE_SCHEMA.update({
    ...
    "EVIDENCED_BY": frozenset({"epistemic"}),
    "BLOCKED_BY": frozenset({"epistemic"}),
    "RUN_OF": frozenset({"lineage"}),
    "FOR_SLATE": frozenset({"lineage"}),
})
...
def _require_identity(value: object, *, label: str) -> dict[str, object]:
    ...
    uri = row["uri"]
    uri_remainder = uri[5:] if isinstance(uri, str) and uri.startswith("gs://") else ""
    ...
    if (
        ...
        or _POSITIVE_INTEGER.fullmatch(generation) is None
...
def validate_node_row(row: Mapping[str, object]) -> dict[str, object]:
    ...
    if kind in OUTCOME_NODE_KINDS:
        _fail(f"outcome node kind {kind} is closed in offline schema v2")
...
def validate_edge_row(row: Mapping[str, object]) -> dict[str, object]:
    ...
    if relationship in OUTCOME_RELATIONSHIP_TYPES:
        _fail(
            f"outcome relationship {relationship} is closed in offline schema v2"
        )
...
def validate_load_manifest(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if not set(namespaces) <= OFFLINE_ALLOWED_NAMESPACES:
        _fail("realized or unknown namespace is closed in offline schema v2")
