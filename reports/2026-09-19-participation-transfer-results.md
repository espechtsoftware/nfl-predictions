...
There are meaningful contest-block losses. **Delivered ranks32–33 lose5.687 expected-max points and0.270percentage points P220** under both audit assumptions. Delivered rank25 gains0.925 expected points but loses0.215percentage points P220 and0.001557 winner proxy. These prevent calling this an improvement for every contest, even under the participation model. Conversely, the first30 delivered rows contain no designated players, so their all-active and participation evaluations are identical. The aggregate benefit mainly comes later.

## Evidence, limits and next concrete step

The protocol and source froze at production `91fff1ed`. Original control ordering reproduces exactly. Synthetic all-active identity, deterministic masks, real-input support and full offline selection/vetting checks pass. A separate legality check finds zero DraftKings or house-strategy violations in all four raw/delivered books. A portable replay independently reopens the same bytes and reproduces every numerical result, exposure, mask, selected roster and delivered order exactly on this laptop. **The workstation independently repeats the complete frozen replay at lab `4845ddb`, with exact agreement.** Its accompanying interpretation accidentally uses raw-order deltas for several delivered-order claims; the correct delivered-block losses above retain the substantive concern without mixing orders.
...
Engineering follow-up: a separate candidate-only adapter now reconstructs the original roster summation order from ordinary saved artifacts, reproduces the D1600 control exactly, and selects the exact same treatment97. Nineteen behavioral tests pass, including stale/future data refusal and confirmed-active overrides. It refuses unresolved Q/D states within90minutes of a player's game instead of guessing after active information should be available. A bounded-memory selector handles synthetic D12800×20,000-world dimensions in62.34seconds using about1.30GiB peak RSS. This is a scale smoke, not an actual D12800 result or rollout.
