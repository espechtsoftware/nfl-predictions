#!/bin/bash
# Run and harvest the one frozen prior-season coverage-fit candidate union.
# Usage: cloud_coverage_tail_union.sh <IMAGE@sha256:...> \
...
RUN_ID=20260810-fp-coverage-tail-union-v1
OUT="$ROOT/reports/coverage-tail-union-runs/$RUN_ID"
...
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable coverage union execution already recorded"; exit 2; }
mkdir -p "$OUT"
printf 'image=%s\nsource=%s\ntreatment=%s\nsource_table=%s\ntreatment_table=%s\nentries=80\ncoverage_candidates=12\ntreated_seasons=2024 2025\n' \
  "$IMG" "$SOURCE" "$TREATMENT" "$SOURCE_TABLE" "$TREATMENT_TABLE" \
  > "$OUT/manifest.txt"

JOB=fantasy-points-coverage-tail-union
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  --image "$IMG" --command nfl-dfs \
  --args "coverage-tail-union,--source-panel,$SOURCE,--treatment-panel,$TREATMENT,--source-table,$SOURCE_TABLE,--treatment-table,$TREATMENT_TABLE" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: coverage union deployed $DEPLOYED, expected $IMG"; exit 1; }

EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: coverage union execution id missing"; exit 1; }
...
gcloud logging read \
  "resource.type=\"cloud_run_job\" AND labels.\"run.googleapis.com/execution_name\"=\"$EXEC\" AND textPayload:\"COVERAGE_TAIL_UNION_JSON=\"" \
...
prefix = "COVERAGE_TAIL_UNION_JSON="
payloads = []
for line in open(sys.argv[1], encoding="utf-8"):
    if prefix in line:
        payloads.append(json.loads(line.split(prefix, 1)[1]))
if len(payloads) != 1:
    raise SystemExit(f"ABORT: expected one coverage union report, got {len(payloads)}")
if not payloads[0].get("disposition"):
    raise SystemExit("ABORT: coverage union report has no disposition")
...
echo "Coverage tail-union confirmation complete: $EXEC ($OUT/report.json)"
