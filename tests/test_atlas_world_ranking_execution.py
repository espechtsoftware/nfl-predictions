...
def test_atlas_runner_queries_are_score_free_and_scope_is_exact():
    ...
    assert "COUNT(DISTINCT source.score_artifact_uri)" in runner.SOURCE_SQL
    assert "COUNT(DISTINCT source.score_artifact_sha256)" in runner.SOURCE_SQL
...
def test_atlas_cloud_contract_is_create_only_and_packaged():
    ...
    finish = (
        ROOT / "scripts/cloud_finish_atlas_world_ranking.sh"
    ).read_text()
    docker = (ROOT / "Dockerfile").read_text()
    assert "20260815-atlas-world-ranking-scorefree-v1/result.json" in launch
    assert "20260815-atlas-world-ranking-scorefree-v1-repair1" in launch
    assert "20260815-atlas-world-ranking-scorefree-v1-repair1" in finish
