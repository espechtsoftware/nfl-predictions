...
ORIGINAL = Path('/home/erich/projects/review-evidence/overnight-20260918/complete-chain-cli')
...
class SnapshotRouter:
    ...
    def __call__(self, sql):
        ...
        assert path.exists() and receipt_path.exists(), 'expanded run refuses new warehouse reads'
...
def main():
    ...
    if arm == 'salaryfix':
        control = pd.read_parquet(ORIGINAL / 'control' / 'training-cache' / 'training_through_2025.parquet')
        pd.testing.assert_frame_equal(control, tr)
    sys.argv = ['scripts/live_week.py', '--season', '2026', '--week', '2', '--group', '153428',
                '--entries', '97', '--lev', '320', '--boom', '1280', '--sims', '10000', '--k', '1',
    ...
    previous = json.loads((ORIGINAL / arm / 'receipt.json').read_text())
    pd.testing.assert_frame_equal(pd.read_parquet(state['out'] / 'frame.parquet'), pd.read_parquet(Path(previous['original_run']) / 'frame.parquet'))
    for label, rec in arrays.items():
        assert rec['sha256'] == previous['arrays'][label]['sha256'], 'expanded budget changed a player bank'
