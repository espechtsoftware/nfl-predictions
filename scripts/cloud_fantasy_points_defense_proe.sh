#!/bin/bash
# Run and harvest the frozen opponent Defense PROE pass-game tail gate.
# Usage: bash scripts/cloud_fantasy_points_defense_proe.sh <IMAGE@sha256:...>
set -euo pipefail

IMG=${1:-}
PROJECT=nfl-predictions-503414
REGION=us-central1
RUN_ID=20260811-fp-defense-proe-l4-v1
SOURCE_RUN=fantasy-points-defense-proe-2022-2025-v1
PANEL=20260810-lockfix-e80-k1-8677d21
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUT="$ROOT/reports/fantasy-points-defense-proe-runs/$RUN_ID"
...
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable Defense PROE execution already recorded"; exit 2; }
...
FROM \`$PROJECT.nfl_raw.fantasy_points_defense_proe\`
" | tail -n 1)
[ "$SOURCE_CHECK" = "2174,4,1,$SOURCE_RUN" ] || {
  echo "ABORT: imported Defense PROE source contract differs: $SOURCE_CHECK"; exit 2; }

mkdir -p "$OUT"
printf 'run_id=%s\nimage=%s\npanel=%s\nsource_run=%s\nsource_rows=2174\nsource_hashes=4\nheld_out=2023 2024 2025\ntarget_weeks=5-18\npositions=QB WR TE\n' \
  "$RUN_ID" "$IMG" "$PANEL" "$SOURCE_RUN" > "$OUT/manifest.txt"

JOB=fantasy-points-defense-proe
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  --image "$IMG" --command nfl-dfs \
  --args "fantasy-points-defense-proe-diagnostic,--panel,$PANEL" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: Defense PROE deployed $DEPLOYED, expected $IMG"; exit 1; }

EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: Defense PROE execution id missing"; exit 1; }
...
gcloud logging read \
  "resource.type=\"cloud_run_job\" AND labels.\"run.googleapis.com/execution_name\"=\"$EXEC\" AND textPayload:\"FP_DEFENSE_PROE_JSON=\"" \
...
prefix = "FP_DEFENSE_PROE_JSON="
payloads = []
for line in open(sys.argv[1], encoding="utf-8"):
    if prefix in line:
        payloads.append(json.loads(line.split(prefix, 1)[1]))
if len(payloads) != 1:
    raise SystemExit(f"ABORT: expected one Defense PROE report, got {len(payloads)}")
if not payloads[0].get("disposition") or not payloads[0].get("gate"):
    raise SystemExit("ABORT: Defense PROE report is incomplete")
...
echo "Defense PROE diagnostic complete: $EXEC ($OUT/report.json)"
