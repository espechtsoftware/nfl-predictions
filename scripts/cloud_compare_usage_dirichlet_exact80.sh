#!/bin/bash
# Compare and harvest the frozen data-fitted K exact-80 books.
# Usage: cloud_compare_usage_dirichlet_exact80.sh <IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
REPORT_FILE="$OUT/usage_dirichlet_exact80_comparison.json"
[ ! -s "$EXEC_FILE" ] || {
  echo "ABORT: immutable fitted-K comparison already recorded"; exit 2; }

JOB=compare-usage-dirichlet-exact80
ARGS="scripts/compare_usage_dirichlet_lineup.py,--historical-source,$HISTORICAL_SOURCE,--evaluation-source,$EVALUATION_SOURCE,--control,$CONTROL,--treatment,$TREATMENT,--experiment-code-sha,$CODE_SHA"
...
grep -q '"disposition"' "$LOG_FILE" || {
  echo "ABORT: structured fitted-K comparison report absent"; exit 1; }
...
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
[ "$STATE" = True ] || { echo "ABORT: fitted-K comparator execution failed"; exit 1; }
echo "Fitted-K exact-80 comparison complete: $REPORT_FILE"
