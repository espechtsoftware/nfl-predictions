#!/bin/bash
# Harvest the frozen, mechanically valid SIS RB run-tail exact-80 report.
set -euo pipefail
...
FILTER="resource.type=\"cloud_run_job\" AND labels.\"run.googleapis.com/execution_name\"=\"$EXEC\" AND textPayload:\"TABPFN_SIS_RB_RUNTAIL_EXACT80_V1_CHUNK=\""
...
prefix = "TABPFN_SIS_RB_RUNTAIL_EXACT80_V1_CHUNK="
chunks = {}
total = None
...
if total is None or set(chunks) != set(range(1, total + 1)):
    raise SystemExit("ABORT: incomplete run-tail report chunks")
...
if report.get("disposition") != "valid" or \
    ...
    raise SystemExit("ABORT: run-tail exact-80 mechanical audit failed")
checks = {
...
for field in ("control_schedules", "treatment_schedules"):
    expected = json.loads(audit.get(f"{field}_json", "{}"))
    got = {str(key): value for key, value in report.get(field, {}).items()}
    if got != expected:
        raise SystemExit(f"ABORT: report {field} differs from manifest")
decision = report.get("result", {}).get("decision", {})
if decision.get("selected_arm") not in {"control", "treatment"}:
    raise SystemExit("ABORT: run-tail decision is not registered")
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    json.dump(report, handle, indent=2, sort_keys=True)
    handle.write("\n")
PY
"$ROOT/.venv/bin/python" - "$OUT/report.json" "$OUT/selected_runtail.txt" "$EXEC" <<'PY'
import json
import sys
...
print("TABPFN_SIS_RB_RUNTAIL_EXACT80_V1_SELECTED", arm, d)
PY
