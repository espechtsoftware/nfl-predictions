...
# Usage: cloud_cbwu_order_invariant_repair.sh <image@sha256:...> <full-code-sha>

PROJECT=nfl-predictions-503414
REGION=us-central1
JOB=cbwu-order-invariant-repair-v1
SERVICE_ACCOUNT=817589974517-compute@developer.gserviceaccount.com
RUN_ID=20260815-cbwu-order-invariant-repair-v1
OUTPUT_URI=gs://nfl-predictions-503414-raw/research/final-forensic-runs/20260814-final-preseason-forensic-v1/post-forensic-addenda/20260815-cbwu-order-invariant-repair-v1/result.json
...
[ "$(sha256sum "$PROTOCOL" | awk '{print $1}')" = "$PROTOCOL_SHA" ] || {
  echo "ERROR: CBWU-OI frozen protocol differs" >&2; exit 2; }
...
[ ! -e "$OUT" ] || {
  echo "ERROR: frozen CBWU-OI run directory already exists: $OUT" >&2; exit 3; }
mkdir -p "$OUT"
printf '%s\n' \
  ...
  'cyclic_comparisons=216' 'candidate_budget=canonical-r0' \
  'tail_line=194' 'selected_entries=80' > "$OUT/manifest.txt"

gcloud run jobs deploy "$JOB" \
  --project "$PROJECT" --region "$REGION" --image "$IMAGE" \
  --command python \
  --args scripts/run_cbwu_seed_order_audit.py,--output-uri,"$OUTPUT_URI",--mode,order-invariant-repair \
...
[ -n "$EXEC" ] || { echo "ERROR: CBWU-OI execution identity missing" >&2; exit 1; }
