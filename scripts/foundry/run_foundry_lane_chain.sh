...
ENV_FILE="$FOUNDRY/foundry_v12${LANE}_env.sh"
RUN_ROOT="$ROOT/reports/corpus-parametric-runs/20260823-foundry-production-v12${LANE}"
...
if [[ ! -s "$RUN_ROOT/governance-live-v12${LANE}/runtime-iam-policy-capture.json" ]]; then
...
run_dir = (
    ...
    f"20260823-foundry-production-v12{lane}/transport-live-v12{lane}"
)
...
path = f"{root}/scripts/foundry/foundry_v12{lane}_env.sh"
body = open(path).read()
