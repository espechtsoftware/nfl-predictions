...
def upcoming_season_week() -> tuple[int, int]:
    """Current season and the next unplayed regular-season week."""
    ...
    return season, int(frame.week.iloc[0])


def sunday_main_group(slates: pd.DataFrame) -> int:
    """Match the UI's Sunday-main rule: largest all-Sunday classic group."""
    ...
    for gid, grp in slates.groupby("draft_group_id", sort=False):
        ...
        days = set(starts.dt.tz_convert("America/New_York").dt.day_name())
        if days != {"Sunday"}:
            continue
    ...
    if not choices:
        raise RuntimeError("no all-Sunday classic draft group is available")
...
def run(*, store=None, generated_at: datetime | None = None) -> dict:
    ...
    if store is None:
        ...
        store = BigQueryStore()
    season, week = upcoming_season_week()
    gid = sunday_main_group(store.classic_slates())
