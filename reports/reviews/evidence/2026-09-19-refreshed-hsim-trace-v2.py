...
def main():
    ...
    manifest_path = here / '2026-09-19-refreshed-hsim-trace-inputs-v2.json'
    ...
    recorded_games = receipt['config']['hsim_game_inputs']
    schedule = pd.DataFrame(recorded_games['games']).rename(columns={
        'home': 'home_team', 'away': 'away_team', 'spread_home': 'spread_line'})
    games = from_live_schedule(frame, schedule, spec['season'], spec['week'])
    assert game_input_receipt(games) == recorded_games
    pilot = []
