#!/usr/bin/env python3
"""Strictly harvest, aggregate, and publish the score-free treatment grid."""
...
from aggregate_stack_core_shell_scorefree import (
    REPORT_VERSION,
    aggregate,
)
import manage_stack_core_shell_scorefree_attempts as attempts
from run_cbwu_seed_order_audit import _upload_create_only
from run_stack_core_shell_scorefree import RUN_ID
...
def _read_ledger(path: Path, fields: int) -> list[list[str]]:
    ...
    if any(len(row) != fields for row in rows):
        raise RuntimeError(f"score-free harvest ledger differs: {path.name}")
    return rows
...
def _gcs_parts(uri: str) -> tuple[str, str]:
    ...
    if match is None:
        raise RuntimeError("score-free harvest URI differs")
    return match.group(1), match.group(2)


def _validate_manifest(out: Path, manifest: dict[str, str]) -> None:
    fixed = {
    ...
    if any(manifest.get(key) != value for key, value in fixed.items()) or \
            manifest.get("finisher_sha256") != _sha(Path(__file__)) or \
            manifest.get("attempt_manager_sha256") != _sha(Path(attempts.__file__)) or \
        ...
        raise RuntimeError("score-free harvest manifest differs")
    for key, relative in (
        ("runner_sha256", "scripts/run_stack_core_shell_scorefree.py"),
        ("aggregator_sha256", "scripts/aggregate_stack_core_shell_scorefree.py"),
        ("source_loader_sha256", "scripts/stack_core_shell_sources.py"),
        ("canary_sha256", "scripts/cloud_wait_stack_core_shell_scorefree_canary.sh"),
        ("canary_validator_sha256", "scripts/validate_stack_core_shell_scorefree_canary.py"),
    ):
        if manifest.get(key) != _sha(ROOT / relative):
            raise RuntimeError(f"score-free harvest source differs: {key}")
    support = ROOT / "reports/stack-core-shell-support-runs" / \
        "20260816-stack-core-shell-control-support-census-v1"
    if _sha(support / "report.json") != manifest["support_report_sha256"] or \
            _sha(support / "completion.txt") != manifest["support_completion_sha256"] or \
            _sha(support / "accepted-executions.txt") != \
            manifest["support_accepted_execution_ledger_sha256"]:
        raise RuntimeError("score-free harvest support binding differs")
...
def finish(out: Path = DEFAULT_OUT) -> dict[str, object]:
    ...
    if not all(path.is_file() for path in required):
        raise RuntimeError("score-free strict harvest receipt is incomplete")
    immutable = [
    ...
    if any(path.exists() for path in immutable):
        raise RuntimeError("immutable score-free strict harvest already exists")
    manifest = _read_manifest(out / "manifest.txt")
    _validate_manifest(out, manifest)
    attempts._configure_base()
    resolution = attempts.validate(out)
    if resolution["disposition"] not in {
        "accepted-primary-population",
        "accepted-population-with-platform-replacements",
    }:
        raise RuntimeError("score-free attempt population is terminally invalid")
    primary = _read_ledger(out / "executions.txt", 5)
    retries = _read_ledger(out / "retry-executions.txt", 6)
    accepted = _read_ledger(out / "accepted-executions.txt", 5)
    if len(primary) != 54 or len(accepted) != 54 or \
            _sha(out / "accepted-executions.txt") != \
            resolution["accepted_execution_ledger_sha256"]:
        raise RuntimeError("score-free accepted execution ledger differs")
    ...
    for season, week, job, execution, uri in accepted:
        ...
        if attempts.base._job_executions(job) != expected_attempts:
            raise RuntimeError("score-free Cloud attempt population differs")
        metadata = attempts.base._execution_metadata(execution)
        attempts._validate_contract(
            metadata, manifest, [season, week, job, execution, uri],
        )
        completed = attempts.base._completed(metadata)
        status = metadata["status"]
        if completed.get("status") != "True" or \
                int(status.get("succeededCount") or 0) != 1 or \
                int(status.get("failedCount") or 0) != 0:
            raise RuntimeError("score-free accepted execution is not successful")
        object_value = attempts.base._object_metadata(uri)
        if object_value is None:
            raise RuntimeError("score-free accepted output object is absent")
    ...
    for (season, week), (uri, object_value) in sorted(objects.items()):
        ...
        if str(blob.generation) != str(object_value.get("generation")) or \
            ...
            raise RuntimeError("score-free shard object changed before download")
        json.loads(raw)
        (shards_pending / f"slate-{season}-{week}.json").write_bytes(raw)

    shard_paths = sorted(shards_pending.glob("slate-*.json"))
    ...
    if report.get("version") != REPORT_VERSION or \
            ...
            report.get("historical_scoring_licensed") is not bool(
                report.get("gate", {}).get("passes_scorefree_gate")
            ) or report.get("disposition") != report.get("gate", {}).get("disposition"):
        raise RuntimeError("score-free aggregate identity or license differs")
    report_raw = (
    ...
    licensed = report["historical_scoring_licensed"]
    completion_lines = [
        "validated_at=" + datetime.now(timezone.utc).isoformat(),
        f"run_id={RUN_ID}", "executions=54", "slates=54", "folds=270",
        ...
        f"historical_scoring_licensed={'true' if licensed else 'false'}",
    ]
    ...
    print("STACK_CORE_SHELL_SCOREFREE_STRICTLY_HARVESTED", report["disposition"])
    return report
