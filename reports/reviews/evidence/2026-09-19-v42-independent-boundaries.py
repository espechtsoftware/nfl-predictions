#!/usr/bin/env python3
"""Independent v4.2 / final provenance boundaries with both provider queries mocked."""
import argparse
import contextlib
import hashlib
...
def main():
    ...
    cases = ["legal_replacement_control", "fresh_csv_out_old_clean_vet", "frame_out_old_clean_vet",
             ...
             "fresh_blank_clears_old_questionable", "wrong_week_flags", "mixed_week_flags",
             "missing_provenance", "rehearsal_nonpublishable"]
    with tempfile.TemporaryDirectory(prefix="v41-review-") as temp:
        for case in cases:
            ...
            flags_frame = pd.read_csv(flags)
            flags_frame["season"] = 2026
            flags_frame["week"] = 2
            flags_frame["draft_group"] = 153428
            if case == "wrong_week_flags":
                flags_frame["week"] = 1
            if case == "mixed_week_flags":
                flags_frame.loc[1, "week"] = 1
            if case == "missing_provenance":
                flags_frame = flags_frame.drop(columns=["season", "week", "draft_group"])
            flags_frame.to_csv(flags, index=False)
            ...
            if case == "rehearsal_nonpublishable":
                argv += ["--test-exclude-dk", "1"]
    ...
    data = dict(kind="offline_synthetic_v42_boundary_review", source_hashes={
