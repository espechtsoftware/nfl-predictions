...
# Usage: cloud_cbwu_seed_order_audit.sh <image@sha256:...> <full-code-sha>

PROJECT=nfl-predictions-503414
REGION=us-central1
JOB=cbwu-seed-order-scorefree-v1
SERVICE_ACCOUNT=817589974517-compute@developer.gserviceaccount.com
OUTPUT_URI=gs://nfl-predictions-503414-raw/research/final-forensic-runs/20260814-final-preseason-forensic-v1/post-forensic-addenda/20260815-cbwu-seed-order-scorefree-v1/result.json
...
[ ! -e "$OUT" ] || {
  echo "ERROR: frozen CBWU run directory already exists: $OUT" >&2
  exit 3
}
mkdir -p "$OUT"
printf '%s\n' \
  ...
  'cyclic_comparisons=216' > "$OUT/manifest.txt"
...
gcloud run jobs deploy "$JOB" \
  ...
  --image "$IMAGE" \
  --command python \
  --args scripts/run_cbwu_seed_order_audit.py,--output-uri,"$OUTPUT_URI" \
...
[ -n "$EXEC" ] || { echo "ERROR: CBWU execution identity missing" >&2; exit 1; }
