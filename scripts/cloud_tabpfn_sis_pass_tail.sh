#!/bin/bash
# Launch the frozen active-only/shared-33 SIS opponent pass-tail cache pair.
# Usage: cloud_tabpfn_sis_pass_tail.sh <GPU_IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
[ ! -e "$OUT/executions.txt" ] || {
  echo "ABORT: immutable SIS pass-tail executions already recorded"; exit 2; }
...
if sched.get("sched_selected") != "false" or \
        ...
        sched.get("cache_table") != expected["cache_table"]:
...
if team.get("team_qb_selected") != "false" or \
        ...
        team.get("cache_table") != expected["cache_table"]:
    raise SystemExit("ABORT: team-QB terminal selection differs")
PY

for table in tabpfn_sis_pass_tail_control_v1 tabpfn_sis_pass_tail_treatment_v1; do
  if bq show --project_id "$PROJECT" "$PROJECT:nfl_features.$table" \
      >/dev/null 2>&1; then
    echo "ABORT: write-once SIS pass-tail table already exists: $table"
    exit 2
  fi
done
...
printf '%s\n' \
  ...
  'source_table=nfl_raw.sis_team_context_game' \
  'source_run=sis-team-context-tranche-1-v1' \
  'positions=QB WR TE' 'window=last four completed games minimum two' \
  'target_seasons=2022 2023 2024 2025' \
  'context=strictly earlier seasons' 'context_max=28000' \
  'random_seed=7' 'n_estimators=4' 'tabpfn_version=2.2.1' \
  'control_table=nfl_features.tabpfn_sis_pass_tail_control_v1' \
  'treatment_table=nfl_features.tabpfn_sis_pass_tail_treatment_v1' \
  > "$OUT/manifest.txt"
: > "$OUT/executions.txt"
...
launch_arm() {
  local arm=$1
  local table=$2
  local job="tabpfn-sis-pass-tail-v1-$arm"
  gcloud run jobs deploy "$job" --project "$PROJECT" --region "$REGION" \
    --image "$IMG" \
    --set-env-vars "GCP_PROJECT=$PROJECT,TABPFN_SIS_PASS_TAIL_ARM=$arm,TABPFN_OUTPUT_TABLE=$table,CODE_SHA=$CODE_SHA" \
...
launch_arm control tabpfn_sis_pass_tail_control_v1
launch_arm treatment tabpfn_sis_pass_tail_treatment_v1
echo "TABPFN_SIS_PASS_TAIL_LAUNCHED $OUT"
