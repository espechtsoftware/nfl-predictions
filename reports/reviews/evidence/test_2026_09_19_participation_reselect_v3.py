...
SOURCE=Path(__file__).with_name('2026-09-19-participation-reselect-v3.py')
...
def test_injury_only_out_cannot_remove_a_dk_questionable_player():
    fr,inj,dk,at,mp,_=fixture();inj.loc[0,'report_status']='Out'
    with pytest.raises(AssertionError,match='injury-only Out'):m.participation(fr,inj,dk,at,mp)


def test_capture_before_announcement_window_cannot_authorize_later_use():
    fr,inj,dk,at,mp,_=fixture()
    dk['game_start']=pd.Timestamp('2026-09-20T17:00:00Z')
    dk['pulled_at']=pd.Timestamp('2026-09-20T15:00:00Z')
    early=m.participation_at_use(fr,inj,dk,mp,as_of='2026-09-20T15:29:00Z')
    assert early[0].tolist()==[.735,1.]
    with pytest.raises(AssertionError,match='confirmation required'):
        m.participation_at_use(fr,inj,dk,mp,as_of='2026-09-20T15:30:00Z')


@pytest.mark.parametrize('case',['wrong_sha','dirty','status_read_error'])
def test_runtime_identity_refuses(monkeypatch,tmp_path,case):
    import subprocess
    def check(args,**kwargs):
        if 'rev-parse' in args:return ('a'*40 if case=='wrong_sha' else m.SOURCE)+'\n'
        if case=='status_read_error':raise subprocess.CalledProcessError(1,args)
        return '?? unexpected.py\n'
    monkeypatch.setattr(m.subprocess,'check_output',check)
    with pytest.raises((AssertionError,subprocess.CalledProcessError)):m.runtime(tmp_path)
