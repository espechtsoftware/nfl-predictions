...
[ -s "$MANIFEST" ] && [ -s "$EXECUTIONS" ] || {
  echo "ABORT: score-free canary launch receipt is incomplete" >&2; exit 2; }
[ "$(wc -l < "$EXECUTIONS")" = 1 ] || {
  echo "ABORT: score-free canary must precede the other 53 cells" >&2; exit 2; }
[ ! -e "$OUT/canary-completion.txt" ] && \
  [ ! -e "$OUT/canary-execution-metadata.json" ] && \
  [ ! -e "$OUT/canary-object-metadata.json" ] || {
  echo "ABORT: immutable score-free canary receipt already exists" >&2; exit 3; }

read -r SEASON WEEK JOB EXEC URI < "$EXECUTIONS"
[ "$SEASON" = 2023 ] && [ "$WEEK" = 1 ] && \
  [ "$JOB" = stack-shell-scorefree-s2023-w1-v1 ] || {
  echo "ABORT: score-free canary identity differs" >&2; exit 2; }

while true; do
  STATE=$(gcloud run jobs executions describe "$EXEC" --project "$PROJECT" \
    --region "$REGION" --format='value(status.conditions[0].status)')
  printf '%s STACK_CORE_SHELL_SCOREFREE_CANARY execution=%s state=%s\n' \
...
[ "$LISTED" = "$EXEC" ] || {
  echo "ABORT: score-free canary job has an extra execution" >&2; exit 2; }
...
[ "$RC" -eq 0 ] || exit "$RC"
echo "STACK_CORE_SHELL_SCOREFREE_REAL_PATH_CANARY_PASSED $EXEC"
