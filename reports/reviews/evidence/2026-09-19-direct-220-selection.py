"""Two preregistered target-tail objectives; fixed inputs, independent cross-audits."""
import hashlib
import json
import runpy
import subprocess
...
def main():
    ...
    books = dict(emax=baseline, wemax=chain['books']['salaryfix_wemax']['candidate_indices'],
                 coverage220=coverage(selection, 97),
                 excess220=list(map(int, select_expected_max(np.maximum(selection - 220, 0), 97))))
    assert all(len(b) == len(set(b)) == 97 for b in books.values())
    print('DIRECT220_BOOKS_FROZEN', flush=True)
    ...
    contrasts = {b+'-minus-emax':{law:{region:{m:uncertainty([
        samples[p][b][region][m]-samples[p]['emax'][region][m] for p in parts])
        for m in samples[parts[0]][b][region]} for region in regions}
        for law,parts in laws.items() if all(b in samples[p] and 'emax' in samples[p] for p in parts)}
        for b in books if b!='emax'}
    result = dict(source_sha256=sha(__file__), input_result_sha256=sha(R/'2026-09-19-repaired-chain-d1600-read.json'),
        ...
        scope='Two frozen objective tradeoffs on one repaired fixed pool; conditional simulation evidence; no outcomes, refit or adoption.')
    with OUT.open('x') as f:
        json.dump(result,f,indent=2,allow_nan=False)
    print(json.dumps(dict(overlap=result['overlap_vs_emax'], elapsed_seconds=result['elapsed_seconds'],
                         repaired_mixture={k:v['prefix97'] for k,v in metrics['salaryfix_equal_mixture'].items()}),indent=2))
