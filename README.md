...
| **Before target Week 5 — SIS pass-tail recurring inputs** | Finish and smoke-test the bounded weekly Fantasy Points W-4…W-1 alignment export plus SIS team pass-defense/pass-rush and Wide/Slot attempt plan. It must append provenance-bearing 2026 rows to `fantasy_points_alignment_player_l4`, `fantasy_points_alignment_team_l4`, `sis_team_context_game`, and `sis_alignment_attempt_game` without changing the historical write-once rows. Until that acquisition passes, the prospective pass-tail jobs fail closed and produce no substitute data. Once available, the GPU control/treatment caches run Thursday at 9:15/9:20am CT and the isolated ten-book paired shadow runs Sunday at 6:00am CT. The money lineup path remains unchanged. |
