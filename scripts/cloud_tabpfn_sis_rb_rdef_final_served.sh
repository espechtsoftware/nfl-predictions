#!/bin/bash
# Run frozen SIS RB run-defense score-free final-served gate.
# Usage: cloud_tabpfn_sis_rb_rdef_final_served.sh <AUDIT_IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable SIS RB gate already recorded"; exit 2; }
...
if cache.get("disposition") != "tabpfn-sis-rb-rdef-caches-valid" or \
        not cache.get("passes") or \
        not cache.get("control_reproduction", {}).get("passes"):
    raise SystemExit("ABORT: SIS RB cache validation did not pass")
...
printf '%s\n' \
  ...
  'primary_position=RB' \
  'primary_gate=aggregate-active-rb-30-point-brier-strictly-improves' \
  'position_factor_grid=0.750:0.005:1.500' 'n_sims=10000' 'seed=0' \
  'blend_model_weight=0.45' > "$OUT/manifest.txt"

ENVS="GCP_PROJECT=$PROJECT,MODEL_ENSEMBLE=1,TABPFN_SIS_RB_RDEF_PANEL_ID=$PANEL"
...
JOB=tabpfn-sis-rb-rdef-final-served-v1
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  --image "$IMG" --command nfl-dfs \
  --args "tabpfn-sis-rb-rdef-final-served,--panel,$PANEL" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: SIS RB gate deployed $DEPLOYED, expected $IMG"; exit 1; }
EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: SIS RB gate execution missing"; exit 1; }
printf '%s\n' "$EXEC" > "$OUT/execution.txt"
echo "TABPFN_SIS_RB_RDEF_FINAL_SERVED_LAUNCHED $EXEC"
