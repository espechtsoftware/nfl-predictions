...
|---|---|---|---|
| 2026-09-04 | The prop-market name resolver used `weekly_stats` as its only GSIS authority, but the current season has no weekly-stat rows before Week 1 is played | A healthy 7,891-row Week-1 prop feed resolved zero players, so live projection silently retained the disclosed DK-PPG fallback instead of activating the adopted props-first blend | **Repaired in source; release pending** — combine historical weekly-stat spellings with current roster/full-football-name aliases and the governed DK-to-GSIS map, exclude normalized names that map to multiple players, then require real Week-1 coverage and immutable-image projection parity before release |
