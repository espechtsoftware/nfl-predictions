...
| **Tue Sep 1** | Purchase the ONE-MONTH Establish The Run DFS subscription (~$45, decided 2026-08-02 — one month only, do NOT auto-renew without review). Covers final camp intel + weeks 1-3, the scheme-change-year window where outside human intel is worth most. Weekly use: download their projections/ownership CSV, upload on the /market page (Consensus diff section, source `etr`); big divergences on players we're heavy on go into the watchlist via chat. Also use their showdown captain-ownership if playing showdown. Cancel-by-default at month end unless the diff flags changed a decision for the better by week 3. |
