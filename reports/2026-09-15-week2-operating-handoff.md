...
|---|---|---|---|---|
| PREREG-099 host driver v2 | `results/run_119_local.pid` in `/home/erich/projects/.nfl2-worktrees/prereg099-supply12800-20260916` (245998 since 2026-09-17 01:32Z; the 00:28Z launch was killed after its first wave collapsed under OpenMP oversubscription, defect 23, and never uploaded a shard) | bank 990 running: run `119b990r1-20260917T013238Z`, 72 slates, 10 workers (amendment 3: the rung is 12,560 candidates — boom is bounded by the 10,000-world bank). **Revised estimate 2026-09-17 12:40Z: no shard yet at 11 h and solves are taking > 60 s each (deep leverage loop under 10-way contention), so a slate costs 15–20 h wall, the whole bank ≈ 4.5–6 days of compute.** With the Saturday 08:00 CT stop and the Sunday pause, expect ≈ 40 of 72 slates by Saturday and the read mid-week (Wed 09-23), not Monday. Nothing downstream depends on it; do not add workers (aggregate solver throughput is the limit, not worker count) | STOP it Saturday 08:00 CT, resume after Sunday | `results/run_119_local.log`, `results/run_119_local_tasks.log`, `results/119_local/<run id>/tNN.log`; run ids in `results/run_119_local_runs.log` | rerun `setsid nohup scripts/run_119_local_v2.sh 10 > results/run_119_local.log 2>&1 < /dev/null &` from that worktree — attach-aware (skips slates already in the bucket) |
