...
| paid-path worktree moved to the level-gate commit | `/home/erich/projects/.nfl2-worktrees/week1-live-center-818f672` @ `818f672` (branch `lab/live-center-production-20260912`); runbook `CLONE`/`EXPECT_SHA` and the Sunday script repointed (main `0354faaf`) | adds a projection-level sanity gate to `live_week.py`: refuses when the naive best-lineup projection is < 135 (verified: lab centering refuses at 112.2, production centering passes at 145.9) and records the number in every receipt. Run dirs copied over; the PREREG cohort worktree (`live-center-production-20260912`) stays on the cohort branch |
