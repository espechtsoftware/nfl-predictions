#!/bin/bash
# Compare and harvest the frozen TabPFN active-label exact-80 books.
# Usage: cloud_compare_tabpfn_active_label_exact80.sh <IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
for panel in "$CONTROL" "$TREATMENT"; do
  grep -q 'ACCEPTANCE PASSED' "$ROOT/reports/panel-runs/$panel/acceptance_check.txt" || {
...
REPORT_FILE="$OUT/tabpfn_active_label_exact80_comparison.json"
[ ! -s "$EXEC_FILE" ] || {
  echo "ABORT: immutable active-label comparison already recorded"; exit 2; }

JOB=compare-tabpfn-active-label-exact80
ARGS="scripts/compare_tabpfn_active_label_lineup.py,--historical-source,$HISTORICAL_SOURCE,--control,$CONTROL,--treatment,$TREATMENT,--experiment-code-sha,$CODE_SHA"
...
grep -q '"disposition"' "$LOG_FILE" || {
  echo "ABORT: structured active-label comparison report absent"; exit 1; }
...
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
[ "$STATE" = True ] || { echo "ABORT: active-label comparator execution failed"; exit 1; }
echo "Active-label exact-80 comparison complete: $REPORT_FILE"
