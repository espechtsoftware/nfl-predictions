...
| §10.2 retrieval presets | `R0-coverage-194` and `R1-strict-200` exist (accepted task-0 laws); `tail-ladder-200-210-220-v1` is a third existing law the roadmap table omits — keep it as an additional comparator; `mean-score-v1` stays the negative control. **R2-expected-max, R3-tail-lcb, R5-regime-robust are the first new retrieval code**, implemented in `corpus_retrieval_engine.py` and registered via the named-scenario registry path integrated at `04d6579`. R4-hybrid-support waits for the §8.2/§8.3 models. |
