...
# The long builds run SATURDAY EVENING after the operator triggers the production refresh at ~17:00 CT
# (`gcloud run jobs execute build-features ... --wait` then `project-slate`), so a failure leaves the whole night:
#   Sat 17:45 CT  D12800 (lev 2560 / boom 10240; delivers 12,560 — one boom solve per simulated world, 10,000 worlds), ≈ 10.7 h -> THE INTENDED ENTRY (operator 2026-09-17; done ≈ 04:30 CT Sun)
#   Sat 17:50 CT  D6400  (lev 1280 / boom 5120),  2 h 08  -> first fallback, on Saturday's refresh
...
# PREREG-099 host bank must be stopped before Saturday 17:45 CT so the cores are free (it resumes later).
...
L12800=(env PAID_LEV=2560 PAID_BOOM=10240 SKIP_PAIR=1 DOSE_FILE=/dev/null "RUN_TAG=$(sat 22:45 d12800sat)" "$DRIVER")
L6400SAT=(env PAID_LEV=1280 PAID_BOOM=5120 SKIP_PAIR=1 DOSE_FILE=/dev/null "RUN_TAG=$(sat 22:50 d6400sat)" "$DRIVER")
