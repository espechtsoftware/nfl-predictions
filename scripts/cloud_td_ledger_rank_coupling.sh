#!/bin/bash
# Launch the sole frozen score-free terminal TD-ledger rank-coupling repair.
# Usage: cloud_td_ledger_rank_coupling.sh <IMAGE@sha256:...> <40-char SHA> [RUN_ID] [JOB]
set -euo pipefail
...
REGION=us-central1
RUN_ID=${3:-20260814-td-ledger-rank-coupling-v1}
JOB=${4:-td-ledger-rank-coupling-v1}
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUT="$ROOT/reports/td-ledger-rank-coupling-runs/$RUN_ID"
PROTOCOL="$ROOT/reports/2026-08-14-td-ledger-rank-coupling-protocol.md"
...
case "$IMG" in *@sha256:*) ;; *) echo "ABORT: immutable rank-coupling image required"; exit 2;; esac
[[ "$CODE_SHA" =~ ^[0-9a-f]{40}$ ]] || {
  echo "ABORT: full immutable rank-coupling code SHA required"; exit 2; }
for path in "$PROTOCOL" "$G0/report.json" "$G0/manifest.txt" \
  "$G1/report.json" "$G1/manifest.txt" "$ACTIVE" "$USAGE"; do
  [ -s "$path" ] || { echo "ABORT: rank-coupling prerequisite missing: $path"; exit 2; }
done
[ ! -e "$OUT" ] || { echo "ABORT: immutable rank-coupling run exists: $OUT"; exit 2; }
...
if g1.get("disposition") != "stable-qb-hub-confirmed" or \
    ...
    raise SystemExit("ABORT: rank-coupling lacks the valid G1 premise")
if active.get("allocation") != "dirichlet" or usage.get("allocation") != "dirichlet":
    raise SystemExit("ABORT: rank-coupling requires accepted finite-K allocation")
...
if set(schedule) != {"2023", "2024", "2025"}:
    raise SystemExit("ABORT: rank-coupling schedule is incomplete")
...
printf '%s\n' \
  ...
  'material_regression_tolerance=log(1.05)' > "$OUT/manifest.txt"

ENVS="GCP_PROJECT=$PROJECT,MODEL_ENSEMBLE=1"
ENVS="$ENVS,TD_LEDGER_RANK_PANEL_ID=${resolved[panel]}"
ENVS="$ENVS,TD_LEDGER_RANK_G0_REPORT_SHA256=$G0_SHA,TD_LEDGER_RANK_G1_REPORT_SHA256=$G1_SHA"
...
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  ...
  --args "td-ledger-rank-coupling,--panel,${resolved[panel]}" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: rank-coupling job deployed $DEPLOYED, expected $IMG"; exit 1; }
EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: rank-coupling execution id missing"; exit 1; }
printf '%s\n' "$EXEC" > "$OUT/execution.txt"
echo "TD_LEDGER_RANK_COUPLING_LAUNCHED $EXEC"
