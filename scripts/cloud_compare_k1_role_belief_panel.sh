#!/bin/bash
# Run and durably harvest the K=1 + CE role-belief comparison in Cloud Run.
# Usage: cloud_compare_k1_role_belief_panel.sh <IMAGE@sha256:...> <union|fixed>
set -euo pipefail
...
REPORT_FILE="$OUT/role_comparison.json"
[ ! -s "$EXEC_FILE" ] || {
  echo "ABORT: immutable comparison execution already recorded"; exit 2; }

JOB=compare-k1-role-panel
ARGS="scripts/compare_k1_role_belief_panel.py,$TREATMENT,--source,$SOURCE,--mode,$MODE"
...
grep -q '"disposition"' "$LOG_FILE" || {
  echo "ABORT: structured role comparison report absent"; exit 1; }
...
with open(sys.argv[2], "w", encoding="utf-8") as fh:
    ...
    fh.write("\n")
PY
echo "K1 role comparison complete: $REPORT_FILE"
