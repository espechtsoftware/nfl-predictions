...
| unique-layout upload slices from the K90 book | `/home/erich/week1-upload-CORRECTED-K90-20260912T1638Z-3df1b0c-{playaction 58-77, ffwc-q6 78-80, ffwc-q5 81-90}.csv` (Milly 1-57 is identical to the K80 file) | **emitted** |
| Sunday autonomous build script | `/home/erich/week1-sunday-build.sh` (runbook build + preflight, K90 build, optional D1600 via `/home/erich/week1-dose.env`, upload CSVs for both layouts into `/home/erich/week1-sunday/`; no publish, no upload) | ready to run by hand at ~09:10 CT; the session's permission classifier refused the systemd timer that would have fired it automatically |
| paid-path worktree moved to the level-gate commit, then to the DST-parity commit | `/home/erich/projects/.nfl2-worktrees/week1-live-center-e7255e9` @ `e7255e9` (branch `lab/live-center-production-20260912`); runbook `CLONE`/`EXPECT_SHA` and the Sunday script repointed (main `221311d2`) | `818f672` adds a projection-level sanity gate to `live_week.py` (refuses when the naive best-lineup projection is < 135; verified: lab centering refuses at 112.2, production centering passes at 145.9). `e7255e9` restores DST parity: the live path had centred DST on a salary-linear prior that ranks the priciest DST first (Chargers 8.03) while the validated panel used production's served DST projection (Jaguars 10.1, Eagles 8.8, Raiders 8.7, Titans 8.2, Ravens 8.1, Chargers 8.0 sixth) — the likely cause of the 41–59/80 Chargers concentration in every corrected book; all 24 slate DSTs match production by `dk_player_id`. `ef7c2c2` on the same branch adds the missing passing-TD market to the lab's market means (Week-2 effect only). Run dirs copied over; the PREREG cohort worktree stays on the cohort branch |
| **DST-fixed corrected pair (the Sunday build's exact code)** | D800 `…/week1-live-center-e7255e9/results/live/2026-w01/20260912T192221336726Z-e7255e9` (sidecars), D400 `…/20260912T192722600030Z-e7255e9`, K90 `…/20260912T193302659236Z-e7255e9` — all `e7255e9`, clean, salary pull 19:04Z | **built** (19:22–19:38Z). D800 receipts: naive top 156.0 (K90 receipt 145.9 — sidecar-free build reports the unblended figure), `matched_skill` 362, `dst_matched` 24/24. Outcome-blind receipts under its own banks: incumbent E[max] 179.5 / P(≥194) .209 / P(≥220) .023 (3df1b0c book: 177.7 / .190 / .018); corrected-hsim 196.4 / .516 / .115 (198.4 / .562 / .133). Book overlap with the 3df1b0c book 1/80 (pool 27/800): the DST change re-orders the boom worlds. Top exposures now Jaguars 56/80, Chase 24, Gibbs 23 (Chargers were 41/80). D400: incumbent 178.1 / .189 / .020. K90: 90 unique rosters, ranks 1–80 identical to the D800 K80 on all nine slots |
| DST-fixed placeholder upload CSVs (draftable ids) | `/home/erich/week1-upload-CORRECTED-DST-20260912T1922Z-e7255e9-{milly-193028206-ranks-1-57, playaction-193028208-ranks-1-20, ffwc-q6-194478066-ranks-1-3, ffwc-q5-194478065-ranks-1-10, all-80-ranks-1-80}.csv` and `/home/erich/week1-upload-CORRECTED-DST-K90-20260912T1933Z-e7255e9-{playaction-193028208-ranks-58-77, ffwc-q6-194478066-ranks-78-80, ffwc-q5-194478065-ranks-81-90}.csv` (receipts `.receipt.json` beside each) | **emitted** 19:41Z; row counts 57/20/3/10/80 and 20/3/10 verified. These supersede the 3df1b0c placeholders (Chargers-centred DST). Not uploaded — DK UI upload is the operator's step |
| PREREG-090 r1 → r2 | lab branch `lab/prereg090-dose1600-directtail-20260912` | mechanics gate r1 PASSED (`lab-run-jdvvj`, 31 min/slate); efficacy bank 900 r1 lost task 36 (2023-W1, 773 players) to the 8 GiB task memory limit → both r1 banks cancelled unread, launcher stopped; runners 110/111/112 now build one float32 decision matrix at a time (amendment 3); r2 image built, jobs updated; r2 cohort (mechanics + 3 banks) re-armed after the local smoke |
| PREREG-091 (experiment 111) | same branch: spread-4 generation (`MAX_PER_GAME=4`, same worlds), column-pricing pass (200 solves on the held-out worlds where the K80 book is weakest), equal-budget dose reference; frozen gates | smoke passed; queued behind PREREG-090 on the lanes |
| PREREG-092 (experiment 112) | same branch: k-means medoid boom schedule (scenario reduction) and its union; frozen gates | written and compiled; smoke pending local CPU |
| live shadow books (corrected centering, flag-gated arms on `lab/live-center-production-20260912` @ `e85c845`; durable copies under `…/live-center-production-20260912/results/live/2026-w01/`) | direct-tail `20260912T181354328816Z-e85c845`, DT-union `…181838197810Z-e85c845`, spread-4 `…182423139959Z-e85c845`, no-bring-back sleeve `…182928000894Z-e85c845` | **built**; scored under the corrected D800's banks (incumbent law, book E[max] / P≥220): D800 177.7/.018 · direct-tail 179.2/.024 · DT-union 179.6/.025 · spread-4 174.5/.009 (but 198.3 = D800 under corrected-hsim) · NOBB sleeve 178.2/.021. Frozen shadows for Monday settlement (`settle_live.py`); never enter the paid book (prereg rules) |
| tail-phenotype census (Neo4j, two cohorts) | `reports/2026-09-12-tail-phenotype-census-two-cohorts.md` | the realized 220+ lineups are disproportionately broad (≥6 games, ≤3 per game) and both pipelines under-select that phenotype — the design rationale for PREREG-091's spread arm |
| DST exposure cap (Step 6) | `<scratch>/exposure_cap_book.py`; frozen shadow `…/20260912T163007999462Z-3df1b0c/shadow_book_dstcap24.json` | measured, **not adopted**: capping the Chargers DST at 24/80 costs 0.3 E[max] and ≈0 at P≥220 under both laws (177.4/.018 vs 177.7/.018; 198.0/.129 vs 198.4/.133), 63/80 overlap. Negligible simulated cost, but an untested selector change the night before lock; the paid book keeps the validated selector and the capped book is settled as a shadow |
