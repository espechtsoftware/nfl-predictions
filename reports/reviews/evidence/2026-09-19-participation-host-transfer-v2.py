...
OUT=ROOT/'participation-host-transfer-v2'
MANIFEST=HERE/'2026-09-19-participation-host-transfer-v2-inputs.json'
...
def inputs():
    ...
    files=[Path(__file__),REPO/'reports/2026-09-19-participation-host-transfer-v2-protocol.md',
...
def vetted(base,s,books,dest):
    """Exactly subset existing captures to the actual frame's smaller ID set."""
    import copy
    source=base.INPUT
    ids=sorted({str(g) for g in s['fr'].gsis_id if str(g) and str(g) not in ('None','nan')})
    groups={}
    for row in json.loads((source/'vetting/query-captures.json').read_text()):
        groups.setdefault(row['sql'],[]).append(row)
    assert len(groups)==3
    new=dest/'derived-vet-inputs';(new/'vetting').mkdir(parents=True)
    records=[]
    for i,(sql,rows) in enumerate(groups.items()):
        def original_ids(r):
            pp=[p for p in r['parameters'] if p['name']=='ids']
            return None if not pp else [x['value'] for x in pp[0]['parameterValue']['arrayValues']]
        eligible=[r for r in rows if original_ids(r) is None or set(ids)<=set(original_ids(r))]
        assert eligible,'new vetter domain is not a subset of any frozen capture'
        r=max(eligible,key=lambda x:len(original_ids(x) or []));fr=pd.read_parquet(base.opened(r))
        params=copy.deepcopy(r['parameters']);prior=original_ids(r)
        if prior is not None:
            assert set(prior)-set(ids)=={'00-0036501'},'unexpected removed ID'
            fr=fr[fr.gsis_id.isin(ids)].reset_index(drop=True)
            next(p for p in params if p['name']=='ids')['parameterValue']['arrayValues']=[{'value':x} for x in ids]
        p=new/'vetting'/f'{i}.parquet';fr.to_parquet(p,index=False)
        records.append(dict(sql=sql,parameters=params,path=str(p),sha256=sha(p),rows=len(fr),
            derived_subset_not_new_capture=True,original_capture=r))
    write(new/'vetting/query-captures.json',records)
    try:
        base.INPUT=new
        return base.vet(s,books,dest/'delivery')
    finally:base.INPUT=source
...
def main(mode):
    ...
    delivery=vetted(base,s,books,dest)
