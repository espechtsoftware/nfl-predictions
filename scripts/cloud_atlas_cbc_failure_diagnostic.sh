...
PREFIX=gs://nfl-predictions-503414-raw/research/atlas-cbc-native-diagnostic-runs/$RUN_ID
...
[ ! -e "$OUT" ] || { echo "ERROR: ATLAS CBC diagnostic local run exists" >&2; exit 2; }
if gcloud storage ls "$PREFIX/**" --recursive --project "$PROJECT" 2>/dev/null | head -1 | grep -q .; then
  echo "ERROR: ATLAS CBC diagnostic cloud run exists" >&2; exit 2
fi
mkdir -p "$OUT"
...
printf '%s\n' \
  ...
  "repair2_image=$IMAGE" "cells=2024-15,2024-16" \
  'uses_realized_outcomes=false' 'persists_lineups=false' \
  > "$OUT/manifest.txt"

: > "$OUT/executions.txt"
for WEEK in 15 16; do
  JOB=atlas-cbc-diag-2024-w${WEEK}-v1
  ...
  gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
    ...
    --set-env-vars CODE_SHA="$CODE_SHA",ANALYSIS_IMAGE="$IMAGE",ATLAS_CBC_DIAGNOSTIC_PROTOCOL_SHA256="$PROTOCOL_SHA",ATLAS_CBC_DIAGNOSTIC_SOURCE_SHA256="$SOURCE_SHA" \
