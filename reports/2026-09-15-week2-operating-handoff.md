...
|---|---|---|---|
| Wednesday, after 05:10 (roster ingest) and after 09:30 (props pull) | assistant | **paid-vendor capture**: `cd <audit worktree> && PYTHONPATH=src /home/erich/projects/nfl-predictions/.venv/bin/python -m nfl_dfs.ops.weekly_vendor_data verify-login`, then `… run --week W --skip-odds` (odds has its own scheduler). Captures the Fantasy Points live matchup reports, imports last week's Route Share and alignment (2026 rows, guarded append-once), runs the SIS pass-tail acquisition. Takes ≈ 15 min unattended once the sessions verify. | run manifest under `fantasy-points/automated/…week-0W/`; `nfl_raw.fantasy_points_route_share` gains rows for season 2026 week W−1 |
