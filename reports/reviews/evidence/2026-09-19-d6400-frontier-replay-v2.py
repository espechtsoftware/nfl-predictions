...
def main():
    ...
    out = args.output
    assert sha(ROOT / 'source/frame.parquet') == '6e56f59d687b8f533ee02c05b90ec2c643c08b0e46aa78155a2c0d7b9894dab6'
    assert sha(ROOT / 'selection-parity/candidate_orders.json') == 'f609fb1324ea10acb56e569ac647a8411f3f36b03e134d0452205422a3c03ad4'
    ...
    map_path = ROOT / 'participation-map.json'
    ...
    for name, r in audit['banks'].items():
        bank_path = ROOT / 'fresh-audit' / (name + '_audit.npy')
        assert sha(bank_path) == r['sha256']
