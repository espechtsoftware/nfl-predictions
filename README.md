...
|---|---|---|---|
| 2026-08-13 | SIS DataHub Pro trial Pass Defense CSV is capped at 20 rows, exports `Rank` as literal `[object Object]`, and contains no week, game, opponent or stable player ID. A requested 2025 Week-1/Split-by-Game retry was byte-identical to the full-season file (same SHA-256) and retained `Games=12..17` | The sample validates useful defender volume/outcome columns but cannot identify a weekly game, support a complete-player panel or be joined safely for PIT modeling. The retry proves the downloaded artifact did not reflect the intended filter, but not yet whether Submit failed to refresh the rendered query or Download returned stale data | **Trial audit open; raw exports isolated** — `/sis/` and `/sis-trial/` are root-gitignored. Pause bulk Pass Defense downloads; after Submit, visually verify whether the rendered table itself changes to `Games=1`. Reject any file whose values remain full-season or whose selected week cannot be encoded externally; stable-ID absence remains a matching-risk finding |
