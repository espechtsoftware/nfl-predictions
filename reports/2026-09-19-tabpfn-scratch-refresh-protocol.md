...
Original support gate: 928 unique Week 2 cache keys (corrected by the pre-read amendment below). Require complete finite means/13 quantiles, nondecreasing quantiles, no duplicate keys, source stability and identical image/source/feature contracts. Report row counts, historical keyed output parity and Week 2 mean/quantile absolute shifts and changed-row counts. Exact historical parity is a diagnostic of the co-run inputs and reproducibility, not a reason to change a threshold after seeing results. If it fails, investigate before attributing upcoming differences to the repair.
...
Launch hardening before any cloud execution: per-execution `TABPFN_OUTPUT_TABLE=scratch_adapter_required` makes the original generator reject the run if command overrides are ignored. Only the validated adapter changes that sentinel to the canonical table name inside its scratch namespace. The [registered launcher](reviews/evidence/2026-09-19-tabpfn-scratch-launch.py) retains its lane on ambiguous launch responses and never repeats a POST; provider reconciliation is required. It records exact operation/execution identities and compares the shared job template before/after every arm.

## Support-gate amendment after the first full execution

`tabpfn-gen-lcf9w` failed before its only write at 02:05:30 UTC because the adapter incorrectly expected 928 **Week 2** rows. The 928-row source table contains **877 Week 2 + 51 Week 3** rows in each arm. All 877 Week 2 rows have eligible QB/RB/WR/TE positions; position filtering was not the cause. The original guard confused whole-table metadata with target-week support. No prediction values were read, neither scratch output table exists, and the shared job template remains identical. This is an invalid support gate, not an adverse model result.

The corrected [preflight](reviews/evidence/2026-09-19-tabpfn-eligible-key-preflight.json) pins the exact same 877 GSIS keys in both arms, SHA `625317ad1854b0221ef2a86ae87e561bea4cdec6b2965d605ea90ca26e317eff`. Both mechanics and refresh now check target-week identity/position support **before inference**, and the sole write requires that exact keyset. The reader requires the same keys. No model, context, feature, random seed or comparison rule changes. Preserve the failed execution and rerun control/repair serially; the original attempt plus two refreshes remains within the one-hour/<$5 planned envelope based on observed runtimes. No automatic launch retry occurred.
