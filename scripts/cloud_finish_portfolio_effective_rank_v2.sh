#!/bin/bash
# Harvest and validate the frozen composite-scope effective-rank diagnostic.
set -euo pipefail
...
if report.get("version") != "v2" or \
        report.get("panel") != "composite-terminal-incumbent-v1":
    raise SystemExit("ABORT: effective-rank report version differs")
historical_panel = manifest.get("historical_panel")
eval_panel = manifest.get("selected_eval_panel")
if report.get("historical_panel") != historical_panel or \
        report.get("selected_eval_panel") != eval_panel:
    raise SystemExit("ABORT: effective-rank composite identity differs")
...
if len(slates) != 107 or \
        {row.get("season") for row in slates} != \
        {2019, 2021, 2022, 2023, 2024, 2025} or \
...
for row in slates:
    expected_panel = historical_panel if row.get("season") in \
        {2019, 2021, 2022} else eval_panel
    if row.get("source_panel") != expected_panel:
        raise SystemExit("ABORT: effective-rank slate source panel differs")
...
with open(sys.argv[3], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
echo "PORTFOLIO_EFFECTIVE_RANK_V2_COMPLETE $OUT/report.json"
