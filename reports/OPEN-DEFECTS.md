...
| O-12 | **No 2026 in-season usage history reaches the model. Root cause: a contract mismatch between the DraftKings ingest and the feature build.** `dk_job.season_week_for` returns `week = None` **deliberately** (its comment: "week resolution happens downstream by joining game_start against nfl_raw.schedules"), so all 722,516 raw 2026 salary rows carry a NULL week — measured, 0 non-null. But `sql/features/001a_dk_salary_week.sql:37` filters `WHERE ... s.week IS NOT NULL`, so **0 rows qualify** and `nfl_features.dk_salary_week` has **zero 2026 rows** (it stops at 2025 week 18). Nothing joins `game_start` to `schedules` in between. The historical `usage` branch in `014_player_week_usage.sql` starts FROM `dk_salary_week`, so week-1 history never enters the rolling windows. | **Every usage feature is null for all 928 Week-2 players and `games_played_prior = 0`.** Verified on the live table, not a stale snapshot: a build completed 2026-09-17T10:18:40Z, four days after Week 1, and produced these zeros, so **the Saturday refresh does not repair it.** Inputs are all present (`weekly_stats` 1,118 wk-1 rows, `snap_counts` 1,492, `player_week_actuals` 1,117, `player_week_role` 921). This is the same condition behind the Week-1 projection-level defect and predates Week 2. **Magnitude of harm to entered projections is NOT established** — production centres on market-blended `proj_points`. | **OPERATOR DECISION, open** | ~~Week 3 deferral~~ **superseded 2026-09-19: the repair is written, reviewed and validated in isolation.** `fix/2026-09-salary-week-resolution` @ `9f218bab`; `001a` sha256 `d9ce2172…f5a8`. Isolated build passed the **unchanged leakage checks**; `dk_salary_week` gains 863 week-1 and 773 week-2 rows; 424 players regain prior-games history; **historical parity exact** (104,847 rows both sides, 0 differences either direction). Measured against a contemporaneous control arm on the original SQL: **mean absolute projection change 0.60 pts, max 3.26, 89 of 472 players over 1 pt, mean signed −0.12**; movement direction coherent (established pass-catchers up, backup/committee runners down). **Not established: that the repaired projections are more accurate** — nothing scores against outcomes. This is production SQL, so it does NOT touch the lab `EXPECT_SHA` identity gate or the armed timers. The remaining question is whether to enter Sunday on a changed book, which is the operator's. |
| O-13 | **`scripts/check_prospective_gates.py` still describes the Route Share gate as "NOT a current-season lever"** after Amendment 1 (2026-09-19) added interim reads after 6 and 10 graded weeks. | The checker's printed guidance contradicts the amended gate; no operational effect on arming. | **Monday 2026-09-22, with O-2** | Registry `adjudicates` / `in_season_value` fields updated to cite Amendment 1, with a test. |
