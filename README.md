...
|---|---|---|---|
| 2026-09-17 | `predictions.player_projections` is append-only: every `project-slate` run adds a full generation for the week (2026 W1 has 18), and a plain `WHERE season=.. AND week=..` mixes generations and doubles the row count | any ad-hoc analysis of "the projections" silently averages superseded runs; the live builder was safe only because it selects `MAX(generated_at)` | accepted by design (the history is the audit trail); consumers read the view `predictions.player_projections_current` (`sql/predictions/003_player_projections_current.sql`), which exposes the newest generation per week plus `generations_for_week` |
| 2026-09-17 | Prop-market blend falls back to DK points-per-game when < 30 % of slate players carry ≥ 2 priced markets; in weeks ≤ 3 of a season DK PPG *is* last week's score, so the 2026-W2 10:36Z generation was `proj_points ≈ 0.77 × week-1 actual` (corr 0.969; top skill projections 9–20 pts above market-implied) | a money book built on such a generation chases last week's box score (one player in 25 of 30 lineups; simulated P(book ≥ 220) 0.73 vs ~0.09 historical) | mitigated: re-run `project-slate` after the day's 09:30 CT props pull and require the log line `market blend source: props` before any build (Week-2 15:02Z generation is clean); a code guard (refuse DK-PPG fallback at week ≤ 3) is queued, not built |
| 2026-09-07 | nflverse published the first 2026 injury artifact before `nflreadpy==0.1.5` advanced its ordinary data-season clock, while the early sparse artifact omitted four nullable columns (`date_modified`, `practice_secondary_injury`, `report_primary_injury`, and `report_secondary_injury`). The collector therefore kept loading completed 2025 injuries and never appended a 2026 collector-time snapshot; after the clock roll, the sparse schema would also have failed the strict snapshot-column check. Separately, freshness treated the one-time, default-off `tabpfn_components` research cache as a recurring live dependency even though production uses `tabpfn_projections` | Week-1 inference had no usable injury/practice/vacated-opportunity context, and the daily freshness check reported both the real empty snapshot feed and a permanent false component-cache alarm | **Bounded production repair implemented and tested locally; deployment/data refresh pending** — during the exact planning-year/roster-year versus prior data-year interval, read only the official planning-year injury path, require a nonempty single-season unique player-week keyset, synthesize NULL only for the four known nullable omissions, and append it with collector time without mixing partial data into the completed-season raw replacement. Keep `injury_snapshots` and adopted `tabpfn_projections` alerting; retain `tabpfn_components` in status as visible research-only/non-alerting. Build an immutable image, run `ingest-nflverse`, then `build-features`, and require `check-freshness` to pass before Week 1; never backdate the new rows |
| 2026-09-04 | The prop-market name resolver used `weekly_stats` as its only GSIS authority, but the current season has no weekly-stat rows before Week 1 is played; after repairing identity, the first live canary exposed a second defect: 234 players currently have only an anytime-TD quote, which the legacy sum treated as a complete fantasy-point market expectation | The zero-match run silently retained DK PPG; the first identity-repaired canary instead activated the slate gate with partial TD-only values and cut several RB/WR projections by 5–6.5 points | **Closed at the live boundary; incomplete feed remains disclosed** — current roster/full-football-name aliases plus the governed map resolve identities without outcomes, while live callers require at least two distinct scoring markets per player before counting a market sum. Immutable generation 64 canary `project-slate-pp2s4` resolved 310 player-weeks, admitted 142 complete rows / 141 live matches (27.9%), correctly stayed below the unchanged 30% slate gate, and wrote a clean 537-row DK-PPG fallback batch. The market feed remains incomplete; no component imputation or threshold reduction is adopted. |
| 2026-09-01 | The lab Neo4j `ProposalAttemptAggregate` export retains only `attempts`, `new`, `dup`, and a reported `reconciles` flag; it omits the raw `infeasible`, `error`, and `exhausted` status counts, and the current lab release does not bind exact source-object identities for the export derivation | Aggregate arithmetic can reconcile while still failing to prove complete attempt-ledger coverage or authoritative source derivation, so treating the four true flags as independently verified would overstate the evidence | **Held as reported-only in Phase B** — the governed adapter records zero verified reconciliations, requires exact external source identities, and says explicitly that source bodies/derivation remain unverified. A later release must bind the complete raw status ledger and source bytes before upgrading reconciliation authority; no graph load or outcome authority is granted |
| 2026-09-01 | The governed 51-roster Millionaire registry and the lab's derivative 48-score file share 46 `(season, week)` keys but disagree on 11 scores by as much as 30.46 points; at least four shared keys contain different nine-player rosters. Multiple Millionaire contests can occur in the same week, neither local source carries a sufficient official contest identity/score receipt, and the v1 registry collapses candidates on `(season, week)` while forcing 17 contests per season | Winner-calibrated utility levels, winner-gap tables, topology conformance, and experiments 080–082 can silently reference different contests or a component-point sum mislabeled as the official winning score. Agreement-only filtering would introduce selection bias and would not establish independent authority | **P0-1 in progress; winner-referenced efficacy held** — preserve v1 and prior results, build a lossless registry-v2 candidate/adjudication ledger with multiple observations per week, separate official target score / captured roster sum / article score, bind a declared target-contest policy and immutable source/adjudication receipts, and fail closed on accepted cohort hash/count. No disputed value is promoted as official; PREREG-080/081/082 remain unscheduled until score-bearing v2 is adjudicated |
| 2026-09-13 | Two historical skill player-weeks carry a placeholder $200 DK salary with a real projection (Melvin Gordon KC 2022 W13, Tony Jones SEA 2022 W6); the replay optimizer used them as free roster spots (136/240 and 80/240 selected lineups on those slates) and both scored 0 | Absolute book level depressed on two of 72 development slates; paired verdicts unaffected | Open: drop skill players with salary < 2,500 from the historical frames under a preregistration amendment (all readers shift together); live DK feeds carry no placeholder salaries |
| 2026-09-13 | DK's full-week classic draft group (`153054`, all 16 Week-1 games) stopped being pulled once Wednesday's game kicked off, but `run_projections.upcoming_slate_features` kept any group with `MAX(game_start) >= now` and preferred the largest group per player, so the projection pool came from a 3-day-old snapshot including already-played teams (Purdy, Smith-Njigba, …) and the fail-closed inference-row check refused every run since 09-08 (57 players); separately `ingest-nflverse` has failed daily since 09-10 because nflverse has not published `ftn_charting_2026.parquet` (404), which aborts the job after rosters/injuries/schedules load | Week-1 books were centred on 09-08 projections until the Sunday fix; nflverse rosters/injuries still refresh (the FTN load is last) but the job reports failure | Fixed: pool keeps only groups whose FIRST game has not started (`HAVING MIN(game_start)`), regression-pinned; FTN 404 tolerance for a new season still open |
| 2026-08-29 | The landed 2026 Week 1 Sunday-main DK group `151307` has 716 salary rows (692 skill players), but only 439 skill rows currently resolve to GSIS identities and 253 do not; `nfl_features.player_week_inference` is empty. The daily NFLverse ingestion has failed since August 25 because upstream `jersey_number` is FLOAT64 while BigQuery expects STRING, and its delete-before-append sequence left `raw.rosters_weekly` with no 2025 or 2026 partition and `raw.player_ids` stale at July 31. nflreadpy 0.1.5 also calls 2025 the active season until opening Thursday and rejects its own already-published 2026 weekly-roster path. Separately, group `151307` is still the August 20 snapshot because the latest green DK job found no upcoming group; score-blind comparison to the August 29 official roster/depth sources proves some remaining rows are stale team/position conflicts or unsupported long snappers rather than safe fuzzy matches | The prospective boom-first runner fails closed in `upcoming_slate_features(2026, 1)` before simulation, so an attempted paired shadow would produce no books or scoring evidence. Weakening identity/team/position requirements, using a salary cutoff, or silently dropping conflicts could omit a legal player or attach a player to the wrong game/model role | **Bounded code repair implemented; immutable build/data refresh pending**: normalize the roster dtype before any delete, restore both 2025 and the exact public 2026 roster-year path with source time/path, ingest 2026 depth snapshots, canonicalize provider aliases, and admit only unique exact roster/depth identities plus five independently reviewed name aliases. The real 2026 roster smoke has 2,930 Week 1 rows/32 teams/2,930 GSIS identities and normalizes cleanly. Build/deploy one pinned image, refresh NFLverse and DK, run the leakage-checked full feature build, and require a fresh DK timestamp, zero unresolved rows over the runner's exact future-classic union, nonzero Week 1 inference and input parity before launch. Current stale team/position conflicts remain fatal; do not use `--full`, `--skip-leakage`, guessed identities, generic fuzzy names, manual zero-fills or outcome-derived repairs |
| 2026-08-29 | There is no canonical immutable historical boom-first bundle that binds an authoritative Sunday-main/common-lock slate index, target-outcome-null features, prior-only training labels, as-of market inputs, TabPFN cache/version, preprojected strictly-prior DST/QB/Vegas inputs, and trained main/role model identities for 2023--2024; there is also no separately frozen normalized actual-points snapshot covering every selected skill and DST identity | The injection-only adapter and paired construction core are fixture-tested, but a real Gate H1 run cannot be launched or graded safely from the existing replay loaders. The legacy DST path reads realized or live-derived inputs; caller-supplied identity strings are not proof of injected bytes; an incomplete slate panel could otherwise look complete; and missing outcome identities would make the grade invalid | **Open -- full source/panel/outcome authority required before H1**: materialize and byte-verify a create-only score-blind composite manifest, require exact authoritative panel membership, freeze main/role model and feature identities, pass an outcome-null real-slate smoke, then freeze all books before creating a separate create-only realized-score snapshot and grader receipt. Add per-seed/leave-one-seed-out, exposure/duplication and descriptive uncertainty reporting before any H1 verdict. Prior-season labels may train later targets, but target-slate outcomes must remain absent until freeze; the adapter rejects 2025 |
| 2026-08-27 | The frozen R6 positive-salary player catalog contains skill-player/week keys that the already-completed realized-outcome query did not return, even though `player_week_actuals` is intended to contain explicit zero labels for salary-listed players without a weekly stat row. The fixed query filters existing source rows instead of left-joining the frozen key spine; the same condition is not valid for DST, whose score is derived from completed schedule/PBP events and can be negative, zero, or positive | The first fixed-job recovery failed before scoring on an inexact player/DST union. Silently treating the returned subset as complete would misstate source evidence; zero-filling a missing DST would corrupt scores | **Bounded R6 correction in progress** — preserve the fixed query/job and its hashes; after exact field/type/micro/duplicate/nonunion/order validation, record observed-versus-synthesized structure, settle only missing skill keys to exact zero under the pinned salary-catalog bridge and salary-zero law, require every DST plus at least one observed skill row per slate, and retain the downstream exact-union validator. Missing DST or a missing whole skill partition remains fatal; this amendment does not call missing players inactive |
...
| 2026-09-12 | The lab's `live_week.py` `book.csv` (Week-1 live pair) still writes stable `dk_player_id` cells, so the file DK-import path documented above would reject it; the Week-1 upload placeholder had been staged from it | Any upload built from `book.csv` fails at DraftKings | Mitigated in production — `scripts/emit_dk_upload_csv_v1.py` emits draftable-ID files from a run dir or a published book; a lab-side importable file must be an additional artifact because `week1_live_pair_adapter.py` maps `book.csv` cells through `dk_player_id` |
...
| 2026-09-12 | Two lab live-path defects found while auditing the Week-1 book: (1) `nfl2/scripts/live_week.py` took its projection LEVEL from the lab component models, which return a per-position constant when the Week-1 usage windows are NULL (every RB/WR ≈ 8 pts, every QB ≈ 14.5), whereas the validated historical panel always centred on production `mean_projection` (`NFL2_CENTER=mean`); (2) `nfl2/src/nfl2/market.py::market_means` sums pass yards, rush/reception yards, receptions and anytime-TD but omits `player_pass_tds` (4 DK pts each), so props-derived QB means run ~35% low (top-12 QB market 12.9 vs production 20.6) | The published 09-10 Week-1 books and every lab live book were built on a collapsed level (naive best-lineup projection 120 vs 144–151 on historical Week-1 slates); under sane beliefs the same selector picks a different book (10/80 overlap, E[max] 171.8 vs 177.7) | **(1) fixed** on lab branch `lab/live-center-production-20260912` (`NFL2_LIVE_CENTER=production` centres live draws on production `player_projections.proj_points`; a projection-level sanity gate refuses a naive top-lineup projection < 135); the Sunday runbook builds from that branch. **(2) fixed** on the same branch (`player_pass_tds` added at 4.0/count); Week-2 effect only, since the paid path is production-centred. Report: `reports/2026-09-12-week1-baseline-fix-and-tonight-steps.md` |
| 2026-09-14 | DraftKings full-standings export quirks: never-filled entries appear with EntryId+Rank and a blank Lineup (1,314 of 832,342 in the Week-1 Millionaire); scores are serialised through single precision (217.05998 for 217.06); small-field ownership rows can be short by one entry for a player rostered in two slots | The full-field validator failed closed on all three real exports (count, rank, ownership cross-check); blank entries now counted toward the field and recorded, ranks reproduced at 2-dp, one-entry gaps below 1% ownership tolerated and recorded; a 167 MB archive needed a chunked upload | Fixed 2026-09-14; first entry-level field ever loaded (`nfl_raw.contest_entries`: 831,028 + 158,302 + 4,998 rows) |
| 2026-09-21 | Fantasy Points live matchup tools answered the same 2026 Week-1 request with two schedules minutes apart (Arizona's opponent alternated between CAR and LAC across 19:01Z-19:40Z on 2026-09-09; Carolina's between ARZ and CHI); one fetch per report could never pass all three schedule gates in one run, and the sealed set was never loaded because the collector had no BigQuery path | Week-1 matchup reports absent from the warehouse; nine runs read as failed although per-report gates passed | Repaired on `research/2026-09-paid-source-preflight`: bounded re-apply per report with every export kept, typed failure classes, `nfl_raw` staging loader (`fantasy_points_*_matchup_weekly`) that also stages the Week-1 seal, shadow-only PIT join; see `reports/2026-09-21-fantasy-points-matchup-capture-incident.md` |
| 2026-09-21 | Fantasy Points matchup exports in the early-season prior-year regime list players by their 2025 aggregates; 3/61 QB and 34/284 WR/TE Week-1 rows (retired or unrostered in 2026 Week 1, e.g. Russell Wilson, Tyreek Hill) resolve to no 2026 roster identity | Those rows stage as `UNRESOLVED:name:pos:teams` and never join a projection row | Accepted: identity is never guessed; the loader audit reports the count per import |
