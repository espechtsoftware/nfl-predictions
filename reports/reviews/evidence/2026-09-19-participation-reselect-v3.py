"""Provider-bound research reselection with runtime and use-time guards (v3).
...
def participation(fr,inj,dk,cutoff,mp,confirmed=None):
    ...
    for n,row in enumerate(fr.itertuples()):
        ...
        assert b!='Out' or a=='Out' or state=='inactive', ('injury-only Out requires official/DK reconciliation',row.id)
        if state=='active':
...
def runtime(lab_checkout):
    lab_checkout=lab_checkout.resolve()
    assert subprocess.check_output(['git','-C',str(lab_checkout),'rev-parse','HEAD'],text=True).strip()==SOURCE
    ...
    import nfl2
    assert Path(nfl2.__file__).resolve().is_relative_to(lab_checkout/'src'),'wrong runtime lab import'
    return lab_checkout


def participation_at_use(fr,inj,dk,mp,confirmed=None,as_of=None):
    at=pd.Timestamp(as_of or datetime.now(timezone.utc))
    p,excluded,records=participation(fr,inj,dk,at,mp,confirmed)
    return p,excluded,records,at


def execute(run,status_dir,map_path,out,confirmed=None,as_of=None,lab_checkout=LAB):
    lab_checkout=runtime(lab_checkout)
    ...
    provider_source=Path(__file__).with_name('2026-09-19-participation-provider-capture.py')
    assert sha(provider_source)=='5fb5e92311fdad7f846858b0ae80655c093d1e2cd2db7a407159b5a760815fc1'
    spec=importlib.util.spec_from_file_location('provider_capture',provider_source)
    provider=importlib.util.module_from_spec(spec);spec.loader.exec_module(provider)
    inj,dk,cutoff,capture=provider.load_capture(status_dir,as_of)
    now=pd.Timestamp(as_of or datetime.now(timezone.utc))
    assert pd.Timestamp(rec['built_utc'])<=cutoff<pd.Timestamp(rec['lock_utc'])
    assert confirmed is None, 'official source acquisition/verification is not implemented; preview only'
    ...
    current=dk.set_index('dk_player_id').loc[fr.dk_player_id]
    assert np.array_equal(current.salary.to_numpy(),fr.salary.to_numpy()), 'salary changed since source build'
    assert np.array_equal(current.dk_draftable_id.to_numpy(),fr.dk_draftable_id.to_numpy()), 'draftable identity changed'
    ...
    p,excluded,records,selection_checked_at=participation_at_use(fr,inj,dk,mp,confirmed,as_of)
    ...
    # The source can age out, a game can lock, or official announcements can
    # become due while a large corpus is being selected. Recheck at output.
    inj2,dk2,cutoff2,capture2=provider.load_capture(status_dir,as_of)
    assert cutoff2==cutoff and capture2==capture
    p2,excluded2,records2,output_checked_at=participation_at_use(fr,inj2,dk2,mp,confirmed,as_of)
    assert np.array_equal(p,p2) and excluded2==excluded and records2==records
    ...
    result=dict(schema='participation-candidate-reselection/v3',disposition='RESEARCH_ONLY_NOT_ADOPTED',
        ...
        status_capture_sha256=sha(status_dir/'capture.json'),status_provider=capture['provider'],
        provider_verifier_sha256=sha(provider_source),
        runtime_lab_checkout=str(lab_checkout),runtime_lab_sha=SOURCE,runtime_lab_clean=True,
        cutoff=str(cutoff),checked_at=str(now),replay_as_of_override=as_of is not None,
        selection_checked_at=str(selection_checked_at),output_checked_at=str(output_checked_at),
        use_time_and_output_time_guards=True,
        ...
        missing_data_policy='unknown labels/practice or missing DK identity refuse; missing injury row uses DK designation and the fixed none-practice map; injury-only Out refuses',
        official_source_implemented=False)
...
if __name__=='__main__':
    ...
    ap.add_argument('--lab-checkout',type=Path,default=LAB)
    a=ap.parse_args();sys.path.insert(0,str(REPO/'src'))
    result=execute(a.run,a.status_dir,a.map,a.output,json.loads(a.confirmed.read_text()) if a.confirmed else None,lab_checkout=a.lab_checkout)
