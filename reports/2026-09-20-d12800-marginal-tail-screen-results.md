...
Use fresh, independently generated selection/audit worlds and freeze the candidate identities before opening any outcome data. Compare the current `dual_emax` book with the existing `cap_prefix_then_fill` ladder (rungs 220/230/240, mean tie-break, and current overlap cap) on the same candidate pool and worlds; use one-row shadows for candidates 1334, 2765, 7399, 5211, and 2396 as mechanism diagnostics. Preserve both bank components and report max mean, P220, P230, P240, row-level rates, roster overlap, and a world-bootstrap interval. Judge the prospective shadow primarily on realized max-of-K and realized 200+/210+ clear counts, with the simulated-to-realized 220 ratio beside the simulated metrics. Keep candidate generation, selection, and first-row contest ordering separate until that paired shadow is complete.

The extreme simulator tail is not assumed calibrated: prior reads found several tail-focused objectives lost on realized panels, while cap-prefix books sometimes improved realized results despite lower simulated tails. No D12800 simulated gain alone authorizes a live selector change; adoption remains under the existing in-season realized-outcome track.
