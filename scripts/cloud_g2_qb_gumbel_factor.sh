#!/bin/bash
# Launch the sole frozen score-free G2 QB-rooted Gumbel factor gate.
# Usage: cloud_g2_qb_gumbel_factor.sh <AUDIT_IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
USAGE_PROTOCOL="$ROOT/reports/2026-08-12-active-label-usage-revalidation-protocol.md"
USAGE_RESULT="$ROOT/reports/2026-08-12-active-label-usage-revalidation-result.md"

case "$IMG" in *@sha256:*) ;; *) echo "ABORT: immutable G2 image required"; exit 2;; esac
case "$CODE_SHA" in
  [0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]*) ;;
  *) echo "ABORT: immutable G2 code SHA required"; exit 2;;
esac
for path in "$PROTOCOL" "$G0/report.json" "$G0/manifest.txt" \
  "$G1/report.json" "$G1/manifest.txt" "$ACTIVE" "$USAGE" \
  "$USAGE_PROTOCOL" "$USAGE_RESULT"; do
  [ -s "$path" ] || { echo "ABORT: G2 prerequisite missing: $path"; exit 2; }
done
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable G2 execution already recorded"; exit 2; }
...
if g1.get("disposition") != "stable-qb-hub-confirmed" or \
    ...
    raise SystemExit("ABORT: G2 lacks a valid G1 license")
if active.get("allocation") != "dirichlet" or \
        usage.get("allocation") != "dirichlet":
    raise SystemExit("ABORT: G2 requires accepted finite-K allocation")
if active.get("selected_k") != usage.get("selected_k"):
    raise SystemExit("ABORT: G2 selected finite K differs")
try:
    if not math.isclose(float(active["selected_k"]), 28.154043586960896,
                        rel_tol=0, abs_tol=0):
        raise ValueError
except (KeyError, ValueError):
    raise SystemExit("ABORT: G2 finite K differs from frozen value")
if active.get("cache_table") != "tabpfn_active_label_treatment_v2":
    raise SystemExit("ABORT: G2 active-label cache differs")
...
print(f"schedule_b64={encode(schedule)}")
PY
)
...
printf '%s\n' \
  ...
  "panel=${resolved[panel]}" "historical_panel=${resolved[panel]}" \
  ...
  "usage_protocol_sha256=$(sha256sum "$USAGE_PROTOCOL" | awk '{print $1}')" \
  "usage_result_sha256=$(sha256sum "$USAGE_RESULT" | awk '{print $1}')" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: G2 job deployed $DEPLOYED, expected $IMG"; exit 1; }
EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: G2 execution id missing"; exit 1; }
printf '%s\n' "$EXEC" > "$OUT/execution.txt"
echo "G2_QB_GUMBEL_FACTOR_LAUNCHED $EXEC"
