...
mkdir -p "$OUT"
PREFLIGHT_JOB=compare-active-label-usage-preflight-v3
...
printf '%s\n' "$PREFLIGHT_EXEC" > "$OUT/comparison_preflight_v3.txt"
while true; do
...
JOB=compare-active-label-usage-revalidation-v3
...
printf '%s\n' "$EXEC" > "$OUT/comparison_execution_v3.txt"
while true; do
