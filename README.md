...
| **Import** (operator-supplied) | `import-ownership`, `import-prop-lines`, `import-sis-*`, `import-fantasy-points-*` | Load licensed vendor CSVs the operator downloads |
...
**Contest entry rosters never landed (2026-08-19).** `ingest/ownership_import.py` parses the ranked entry block (rank, entry name, full lineup) from a DK contest-standings export and loads it *first*, but `contest_entries` has never received a row — only `contest_ownership` is populated (2022–2025, 18 weeks per season). DK purges standings exports after ~4 days, so historical field rosters are unrecoverable; the 51 tracked Millionaire winners are the entire observation of the field for those seasons. The September Monday/Tuesday standings downloads are therefore load-bearing: they are the only path to a measured effective field size, a field model, and a winner set larger than the tracked first-place rosters.
