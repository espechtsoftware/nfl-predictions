...
def inventory() -> dict[str, object]:
    return generate_effective_policy_rule_inventory_v7(ROOT)


def test_v7_is_explicit_and_v5_literals_remain_frozen(inventory) -> None:
    ...
    assert inventory["source_set_id"] == V7_SOURCE_SET_ID
...
"""
    ...
    assert archived["inventory_sha256"] == (
        "2bf2eebd4bf17500f5fa09ecadbc6b76ddf14410678a4e26ebda34811e728ffa"
    )
    assert archived["policy_semantics_sha256"] != canonical_sha256(
        _policy_semantics(inventory)
    )
    assert archived["projection_sha256"] == CLASSIFIED_INPUT_PROJECTION_SHA256
    assert archived["source_set_id"] == SOURCE_SET_ID
    assert archived["source_set_sha256"] == (
        "926b776a2f233e3a32db1395b9e592eac2013334abd192421558dfa87239039f"
    )


def test_cross_label_rehash_cannot_turn_v7_into_v5(inventory) -> None:
...
def test_every_rule_has_a_separate_typed_dose_path_and_source(inventory):
    ...
    for row in rules:
        ...
        assert all(locator["path"] in V7_FROZEN_SOURCE_SHA256
...
def test_runtime_input_projection_is_an_exact_classified_partition(inventory):
    ...
    for key, row in inputs.items():
        ...
        assert all(
            site["source_sha256"]
            == V7_FROZEN_SOURCE_SHA256[site["path"]]
                   for site in row["direct_read_sites"])
...
def test_source_hash_drift_fails_before_policy_import(tmp_path: Path):
    for relative in V7_FROZEN_SOURCE_SHA256:
    ...
    with pytest.raises(
        ...
        generate_effective_policy_rule_inventory_v7(tmp_path)
...
def test_source_classifier_omission_and_reclassification_fail_closed(monkeypatch):
    ...
    with pytest.raises(
        ...
        generate_effective_policy_rule_inventory_v7(ROOT)
