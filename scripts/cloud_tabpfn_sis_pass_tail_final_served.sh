#!/bin/bash
# Run the frozen SIS opponent pass-tail score-free final-served gate.
# Usage: cloud_tabpfn_sis_pass_tail_final_served.sh <AUDIT_IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
[ ! -e "$OUT/execution.txt" ] || {
  echo "ABORT: immutable SIS pass-tail gate already recorded"; exit 2; }
...
if cache.get("disposition") != "tabpfn-sis-pass-tail-caches-valid" or \
        not cache.get("passes") or \
        not cache.get("control_reproduction", {}).get("passes"):
    raise SystemExit("ABORT: SIS pass-tail cache validation did not pass")
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: SIS pass-tail gate deployed $DEPLOYED, expected $IMG"; exit 1; }
EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: SIS pass-tail gate execution missing"; exit 1; }
printf '%s\n' "$EXEC" > "$OUT/execution.txt"
echo "TABPFN_SIS_PASS_TAIL_FINAL_SERVED_LAUNCHED $EXEC"
