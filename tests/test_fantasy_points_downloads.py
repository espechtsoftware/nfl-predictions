...
def test_load_checked_in_same_season_qb_shell_fit_plan():
    ...
    payload, specs = load_plan(
        ...
        / "fantasy_points"
        / "plans"
        / "same-season-qb-shell-fit-last-four-v1.json"
    )
    assert payload["name"] == "same-season-qb-shell-fit-last-four-v1"
    assert len(specs) == 56
    assert {spec.report for spec in specs} == {"coverage-matrix"}
    assert all(spec.context == "Offense" for spec in specs)
    assert all(spec.target_week in range(5, 19) for spec in specs)
    assert all(
        spec.weeks == tuple(range(spec.target_week - 4, spec.target_week))
        for spec in specs
