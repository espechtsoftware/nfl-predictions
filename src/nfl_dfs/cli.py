...
def main(argv: list[str] | None = None) -> None:
    ...
    sub.add_parser(
        "shadow-cbwu-volume",
        help=(
            "Freeze the paired volume-OI admission shadow: twenty seed "
            "books' candidates admitted at the registered budget"
        ),
    )
    sub.add_parser(
    ...
    sub.add_parser(
        "sis-pass-tail-support-audit",
        help="Outcome-free support/redundancy audit for SIS pass-tail context",
    )
    ...
    p = sub.add_parser(
        "import-fantasy-points-alignment-weekly",
        help="Audit/append one W-4..W-1 prospective alignment window",
    )
    p.add_argument("--input-dir", required=True)
    p.add_argument("--target-week", required=True, type=int)
    p.add_argument(
    ...
    p = sub.add_parser(
        "served-tail-calibration-diagnostic",
        help="Measure q90/q95/q99 calibration on the final served draws",
    )
    p.add_argument(
        "--panel", default="20260810-lockfix-e80-k1-8677d21",
        help="Frozen corrected K1 panel (alternate values fail closed)",
    ...
    p = sub.add_parser(
        "served-tail-recalibration-diagnostic",
        help="Fit and gate the frozen mean-invariant served-tail correction",
    )
    p.add_argument(
        "--panel", default="20260810-lockfix-e80-k1-8677d21",
        help="Frozen corrected K1 panel (alternate values fail closed)",
    ...
    p = sub.add_parser(
        "route-rank-dependence-r2",
        help="Gate fixed midpoint Route ranks on G0/G1 dependence",
    )
    p.add_argument("--panel", required=True)
    ...
    p = sub.add_parser(
        "import-fantasy-points-qb-shell",
        help="Audit/import frozen offense and defense QB shell windows",
    )
    p.add_argument("--input-dir", required=True)
    p.add_argument("--defense-input-dir", required=True)
    p.add_argument(
        "--write", action="store_true",
        help="Create the private raw table; existing non-identical data aborts",
    )
    ...
    p = sub.add_parser(
        "fantasy-points-qb-shell-diagnostic",
        help="Run the frozen QB offense-by-defense shell-fit gate",
    )
    p.add_argument(
        "--panel", default="20260810-lockfix-e80-k1-8677d21",
        help="Frozen corrected K1 panel (alternate values fail closed)",
    ...
    if args.command == "ingest-nflverse":
    ...
    elif args.command == "shadow-cbwu-volume":
        from .inference import prospective_shadow
        ...
        sis_team_run_context.run()
    elif args.command == "sis-pass-tail-support-audit":
        from .analysis import sis_pass_tail_context

        sis_pass_tail_context.run()
    ...
    elif args.command == "import-fantasy-points-alignment-weekly":
        from .ingest import fantasy_points_alignment_weekly

        fantasy_points_alignment_weekly.run(
            args.input_dir,
            target_week=args.target_week,
            write=args.write,
        )
    ...
    elif args.command == "served-tail-calibration-diagnostic":
        from .analysis import served_tail_calibration

        served_tail_calibration.run(args.panel)
    elif args.command == "served-tail-recalibration-diagnostic":
    ...
    elif args.command == "route-rank-dependence-r2":
        from .analysis import route_rank_dependence_r2

        route_rank_dependence_r2.run(args.panel)
    ...
    elif args.command == "sis-receiver-copula-heldout":
    ...
    elif args.command == "import-fantasy-points-qb-shell":
        from .ingest import fantasy_points_qb_shell

        fantasy_points_qb_shell.run(
            args.input_dir, args.defense_input_dir, write=args.write)
