...
|---|---|---|---|
| 2026-08-13 | The first SIS DataHub Pro trial pass-defense CSV is capped at 20 rows, exports `Rank` as the literal string `[object Object]`, and contains no week, game, opponent or stable player ID even though `Split by Game` was checked in the visible filter state; the rendered/downloaded rows still show full-season `Games=12..17` aggregates | The sample validates useful defender volume/outcome columns but cannot identify a weekly game, support a complete-player panel or be joined safely for PIT modeling. Visible checkbox state alone cannot prove that Submit refreshed the result before download | **Trial audit open; raw export isolated** — `/sis/` and `/sis-trial/` are root-gitignored. Re-run 2025 Week 1 to Week 1 with `Split by Game`, press `Submit`, verify rendered `Games=1`, then download. Reject any file whose values remain full-season or whose selected week cannot be encoded externally; stable-ID absence remains a matching-risk finding |
