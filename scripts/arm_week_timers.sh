...
# PREREG-099 host bank must be stopped before Saturday 19:00 CT so the cores are free (it resumes later).
set -euo pipefail
WEEK=${1:?week}; RUN=${2:-}
...
L12800=(env PAID_LEV=2560 PAID_BOOM=10240 SKIP_PAIR=1 DOSE_FILE=/dev/null "RUN_TAG=$(tag 00:00 d12800sat)" "$DRIVER")
L6400SAT=(env PAID_LEV=1280 PAID_BOOM=5120 SKIP_PAIR=1 DOSE_FILE=/dev/null "RUN_TAG=$(tag 00:05 d6400sat)" "$DRIVER")
...
cat <<EOT
# Week $WEEK (America/Chicago): Sat $SATURDAY 17:30 operator refresh (two gcloud lines), 19:00 D12800, 19:05 D6400 insurance;
...
systemd-run --user --on-calendar="$SATURDAY 19:00 America/Chicago" --unit $U12800 ${L12800[*]}
systemd-run --user --on-calendar="$SATURDAY 19:05 America/Chicago" --unit $U6400SAT ${L6400SAT[*]}
