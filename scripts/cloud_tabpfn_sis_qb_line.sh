#!/bin/bash
# Launch the frozen active-only/shared-33 SIS QB line cache pair.
# Usage: cloud_tabpfn_sis_qb_line.sh <GPU_IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
[ ! -e "$OUT/executions.txt" ] || {
  echo "ABORT: immutable SIS QB line executions already recorded"; exit 2; }
...
for table in tabpfn_sis_qb_line_control_v1 tabpfn_sis_qb_line_treatment_v1; do
  if bq show --project_id "$PROJECT" "$PROJECT:nfl_features.$table" \
      >/dev/null 2>&1; then
    echo "ABORT: write-once SIS QB line table already exists: $table"
    exit 2
  fi
done
...
launch_arm() {
  local arm=$1
  local table=$2
  local job="tabpfn-sis-qb-line-v1-$arm"
  gcloud run jobs deploy "$job" --project "$PROJECT" --region "$REGION" \
    --image "$IMG" \
    --set-env-vars "GCP_PROJECT=$PROJECT,TABPFN_SIS_QB_LINE_ARM=$arm,TABPFN_OUTPUT_TABLE=$table,CODE_SHA=$CODE_SHA" \
...
launch_arm control tabpfn_sis_qb_line_control_v1
launch_arm treatment tabpfn_sis_qb_line_treatment_v1
echo "TABPFN_SIS_QB_LINE_LAUNCHED $OUT"
