#!/bin/bash
# Run the frozen adaptive SIS RB opponent run-tail final-served gate.
# Usage: cloud_tabpfn_sis_rb_runtail_final_served.sh <AUDIT_IMAGE@sha256:...> <40-char SHA>
set -euo pipefail
...
case "$IMG" in *@sha256:*) ;; *) echo "ABORT: immutable run-tail audit image required"; exit 2;; esac
[[ "$CODE_SHA" =~ ^[0-9a-f]{40}$ ]] || {
  echo "ABORT: full immutable run-tail code SHA required"; exit 2; }
for path in "$PROTOCOL" "$AUDIT" "$AUDIT_SQL" "$ACTIVE" "$USAGE" "$CACHE"; do
  [ -s "$path" ] || { echo "ABORT: run-tail prerequisite missing: $path"; exit 2; }
done
[ ! -e "$OUT" ] || {
  echo "ABORT: immutable run-tail final-served run exists: $OUT"; exit 2; }
...
printf '%s\n' \
  ...
  'primary_gate=equal-q95-q99-normalized-pinball-mean-ratio-below-one' \
  'position_factor_grid=0.750:0.005:1.500' 'n_sims=10000' 'seed=0' \
  'blend_model_weight=0.45' > "$OUT/manifest.txt"

ENVS="GCP_PROJECT=$PROJECT,MODEL_ENSEMBLE=1,TABPFN_SIS_RB_RUNTAIL_PANEL_ID=$PANEL"
...
JOB=tabpfn-sis-rb-runtail-final-served-v1
gcloud run jobs deploy "$JOB" --project "$PROJECT" --region "$REGION" \
  --image "$IMG" --command nfl-dfs \
  --args "tabpfn-sis-rb-runtail-final-served,--panel,$PANEL" \
...
[ "$DEPLOYED" = "$IMG" ] || {
  echo "ABORT: run-tail gate deployed $DEPLOYED, expected $IMG"; exit 1; }
EXEC=$(gcloud run jobs execute "$JOB" --project "$PROJECT" --region "$REGION" \
  --async --format='value(metadata.name)')
[ -n "$EXEC" ] || { echo "ABORT: run-tail gate execution missing"; exit 1; }
printf '%s\n' "$EXEC" > "$OUT/execution.txt"
echo "TABPFN_SIS_RB_RUNTAIL_FINAL_SERVED_LAUNCHED $EXEC"
