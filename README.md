...
|---|---|---|---|
| 2026-08-10 | Fantasy Points Data Suite Advanced Receiving/Rushing Player exports are full-season aggregates with no week column; Advanced Rushing also emits two distinct six-column subgroups with duplicate column names when `Include Group Headers` is disabled | Same-season use would leak future games, and parsing the duplicate rushing headers by name would silently overwrite one subgroup or assign it the wrong meaning | **Guarded at intake** — Advanced Receiving is eligible only as a strict prior-season feature family; the weekly Route/Target/Snap reports remain the same-season sources. Re-export one Advanced Rushing sample with group headers enabled before accepting that family, retain source hashes, and fail any importer on duplicate semantic columns |
