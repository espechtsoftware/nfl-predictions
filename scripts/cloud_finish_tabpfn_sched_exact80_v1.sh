#!/bin/bash
# Validate, compare and select the PIT-clean TabPFN SCHED exact-80 arm.
# Usage: cloud_finish_tabpfn_sched_exact80_v1.sh <AUDIT_IMAGE@sha256:...>
set -euo pipefail
...
for path in "$TIER1" "$USAGE" "$ACTIVE" "$FINAL" "$CACHE"; do
...
INHERITED_TABLE=$(awk -F= '$1=="cache_table" {print $2}' "$ACTIVE")
...
read -r CONTROL_B64 TREATMENT_B64 <<< "$("$ROOT/.venv/bin/python" - "$FINAL" "$CACHE" <<'PY'
import base64
import json
import sys
...
mkdir -p "$OUT"
JOB=compare-tabpfn-sched-exact80-v1
ARGS="scripts/compare_tabpfn_sched_lineup_v1.py,--historical-source,$SOURCE,--historical-code-sha,a12ab31,--control,$CONTROL,--treatment,$TREATMENT,--code-sha,$CODE_SHA,--role-selected,$ROLE_SELECTED,--allocation,$ALLOCATION,--selected-k,$SELECTED_K,--control-schedules-b64,$CONTROL_B64,--treatment-schedules-b64,$TREATMENT_B64,--cache-validation-sha,$CACHE_SHA,--final-served-sha,$FINAL_SHA"
...
FILTER="resource.type=\"cloud_run_job\" AND labels.\"run.googleapis.com/execution_name\"=\"$EXEC\" AND textPayload:\"TABPFN_SCHED_STAGE_B_V1_JSON=\""
...
if len(payloads) != 1:
    raise SystemExit(f"ABORT: expected one SCHED comparison, got {len(payloads)}")
report = payloads[0]
if report.get("disposition") != "valid" or report.get("failures"):
    raise SystemExit("ABORT: SCHED comparison is invalid")
...
if selected not in {sys.argv[2], sys.argv[3]}:
    raise SystemExit("ABORT: SCHED selection is not registered")
print(selected)
PY
)
if [ "$SELECTED" = "$TREATMENT" ]; then
  bash "$ROOT/scripts/cloud_accept_panel.sh" "$IMG" "$TREATMENT" promote 80 2 \
    "2023 2024 2025" season-varying-config
  SCHED_SELECTED=true
  FEATURE_CONTRACT=sched35
  CACHE_TABLE=tabpfn_sched_treatment_v1
  SELECTED_EVAL=$TREATMENT
else
  SCHED_SELECTED=false
  FEATURE_CONTRACT=shared33
  CACHE_TABLE=$INHERITED_TABLE
  SELECTED_EVAL=$SOURCE
fi
printf '%s\n' \
  "historical_source=$SOURCE" "role_selected=$ROLE_SELECTED" \
  "allocation=$ALLOCATION" "selected_k=$SELECTED_K" \
  "label_law=$LABEL_LAW" "sched_selected=$SCHED_SELECTED" \
  "feature_contract=$FEATURE_CONTRACT" "cache_table=$CACHE_TABLE" \
...
echo "PIT_SCHED_EXACT80_SELECTED $SELECTED feature_contract=$FEATURE_CONTRACT"
