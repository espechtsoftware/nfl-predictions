...
|---|---|---|---|
| Wednesday, after 05:10 (roster ingest) and after 09:30 (props pull) | assistant | **paid-vendor capture**: `cd <audit worktree> && PYTHONPATH=src /home/erich/projects/nfl-predictions/.venv/bin/python -m nfl_dfs.ops.weekly_vendor_data verify-login`, then `… run --week W --skip-odds --no-login-if-needed` (odds has its own scheduler). **Mode (2026-09-23):** `run` defaults to ATTENDED mode, which forces a fresh SIS login and prompts for the email and password at the terminal; from the assistant's session (no keyboard) that stops at `EOFError: EOF when reading a line`. The assistant therefore always passes `--no-login-if-needed` (unattended: verifies the saved Fantasy Points session and, before week 5 with no `--sis-plan`, records SIS as `not-required`; from week 5 it verifies the saved SIS session). The operator runs the attended form only when renewing a session. **What runs when (every paid family with history, one run = everything; 2026-09-23):** every week — Fantasy Points live matchup reports (line / QB coverage / WR coverage) captured **and staged** into `nfl_raw.fantasy_points_*_matchup_weekly` (`fantasy-points-matchups-stage`); **from week ≥ 2** — Route Share and **Defense PROE** (one completed week W−1 each, guarded append-once into `fantasy_points_route_share` / `fantasy_points_defense_proe`); **from week ≥ 5 only** — the W−4…W−1 families: alignment, advanced passing, route shape, coverage (receiver + defense), QB shell (parsed with that week's coverage run), advanced receiving (cumulative 1…W−1 from week 5; last-four window added from week 6), and the SIS pass-tail acquisition (they need four prior weeks; their absence from an earlier manifest is correct, not a failure). `--audit-only-fp-families` / `--skip-fp-families` / `--audit-only-matchups` / `--skip-matchup-stage` for a validation-only or partial run. Defense PROE backfill of 2026 Weeks 1–2 (one-off): see `src/nfl_dfs/ingest/fantasy_points_defense_proe_weekly.py`. The `*_prior` tables need nothing in 2026 (a 2026 target reads season-2025 rows, already loaded). **SIS team context (2026-09-23; operator: SIS every week, no silent fallbacks):** without `--sis-plan` the run captures and imports the tracked `automation/sis/plans/team-context-2026-wNN.json` for the completed week W−1 (`sis-team-context-import`; plans are tracked for weeks 2–17). Every Fantasy Points step runs FIRST; the SIS session is verified immediately before the first SIS step. An expired session or a missing plan **fails the run** (non-zero exit) with the fix: `sis-download login --terminal-credentials --fresh`, then `nfl-weekly-data run --week W --skip-fantasy-points --skip-odds --skip-matchups --no-login-if-needed` (SIS only; the FP files are already captured). **Weeks whose matchups were captured by schema-1 runs** (pre-2026-09-23): `python -m nfl_dfs.ops.fantasy_points_matchup_seal --output-root fantasy-points/automated --week W --out reports/<date>-weekW-fantasy-points-live-matchup-capture-seal.json`, then `python -m nfl_dfs.ingest.fantasy_points_matchups_weekly --input <seal> --target-week W --output-root fantasy-points/automated --write`. A `--sis-plan` run is on demand only. ≈ 2 min at week 2, longer from week 5. **Superseded 2026-09-21 (defect 28 fix):** the SIS session is required only when an SIS step runs (an approved plan, or the pass-tail acquisition from week 5); before that an unattended run records SIS as `not-required`. An expired session still stops any run that needs it. | the run manifest `weekly-data-runs/<stamp>__season-2026-week-0W/manifest.json` has `status: complete` with every expected step `complete`; `nfl_raw.fantasy_points_route_share` and `fantasy_points_defense_proe` gain rows for season 2026 week W−1; the three `fantasy_points_*_matchup_weekly` tables gain target week W; from week 5 the alignment, `advanced_passing_l4`, `route_shape_l4`, `receiver_coverage_l4`, `defense_coverage_l4`, `qb_shell_l4` and `advanced_receiving_windows` tables gain season 2026 target week W |
