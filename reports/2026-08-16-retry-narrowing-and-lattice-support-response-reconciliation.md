...
2. Repair3 was a launch-namespace defect that no retry should conceal. Both
   future 54-cell lattice populations now use their actual 2023 Week 1 cell,
   job name, command and immutable prefix as a canary. The remaining 53 cells
   cannot launch until its exact execution specification, terminal success
   and positive object metadata pass without downloading the object.
...
The support and scientific launchers bind these sources, execute the canary
before the release loop, and retain a 53-cell grid-release receipt. Their
finishers require the canary evidence and an exact accepted-attempt population
before any shard is downloaded. The support aggregator emits the added
distribution diagnostics and the strict finisher validates their full schema.
