#!/bin/bash
# Compare the corrected common-lock K1 control with its fixed-budget CE12 arm.
# Usage: cloud_compare_corrected_k1_ce_panel.sh <IMAGE@sha256:...>
set -euo pipefail
...
TREATMENT=20260810-lockfix-e80-k1-ce12-8677d21
ROOT=$(cd "$(dirname "$0")/.." && pwd)
OUT="$ROOT/reports/panel-runs/$TREATMENT"

case "$IMG" in *@sha256:*) ;; *)
  echo "ABORT: immutable comparator image required"; exit 2 ;;
esac
[ -d "$OUT" ] || { echo "ABORT: panel directory absent: $OUT"; exit 2; }
EXEC_FILE="$OUT/comparison_execution.txt"
LOG_FILE="$OUT/comparison_log.json"
REPORT_FILE="$OUT/ce_comparison.json"
[ ! -s "$EXEC_FILE" ] || {
  echo "ABORT: immutable comparison execution already recorded"; exit 2; }

JOB=compare-corrected-k1-ce-panel
ARGS="scripts/compare_k1_ce_panel.py,$TREATMENT,--source,$SOURCE,--mode,fixed"
...
with open(sys.argv[2], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
echo "Corrected K1 CE comparison complete: $REPORT_FILE"
