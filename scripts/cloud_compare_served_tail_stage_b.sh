#!/bin/bash
# Compare the sole served-tail Stage B treatment and harvest its report.
# Usage: cloud_compare_served_tail_stage_b.sh <IMAGE@sha256:...> <CODE_SHA>
set -euo pipefail
...
grep -q 'ACCEPTANCE PASSED' "$OUT/acceptance_check.txt" || {
  echo "ABORT: partial-panel check-only acceptance has not passed"; exit 2; }
...
[ ! -s "$EXEC_FILE" ] || {
  echo "ABORT: immutable Stage B comparison already recorded"; exit 2; }

JOB=compare-served-tail-stage-b
ARGS="scripts/compare_served_tail_lineup.py,$TREATMENT,--source,$SOURCE,--treatment-code-sha,$CODE_SHA"
...
grep -q '"disposition"' "$LOG_FILE" || {
  echo "ABORT: structured Stage B comparison report absent"; exit 1; }
...
echo "Served-tail Stage B comparison complete: $REPORT_FILE"
